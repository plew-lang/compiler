...
for source in tests/run/try_from_conversion.pw tests/run/try_from_error_overload.pw tests/run/try_err_unwind_arc.pw; do
    echo "check $source(try Mid CFG)" >&2
    "$PLEWC" --emit-mid-coverage "$source" >"$directory/input.ll" 2>"$directory/coverage"
    if grep -E 'name=(process|outer|stage) category=' "$directory/coverage" >&2; then
