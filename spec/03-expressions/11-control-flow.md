...
- **or パターン `Pat1 | Pat2 | …`** — `|` で並べた選択肢の**どれかに一致すれば成立**し、1 つのアーム本体を共有します（`Color.Red | Color.Green => …`）。網羅性は列挙した全バリアントを数えます。**各選択肢は同じ名前・同じ型を束縛**しなければなりません（本体から見える束縛が一意に定まるため）。nullary バリアントは束縛を持たないので自由に並べられます（最頻ケース）。`|` はパターン位置でのみ区切りで、`=>` 以降の本体に書く `|` は通常のビット OR です。
