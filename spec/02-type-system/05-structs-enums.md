...
構築する型が**文脈から一意に定まる**ときは、JSX の**メンバ名の前の型名**を省いて **`<.Member …/>`・`<.member …/>`** と書けます（列挙型バリアント・名前付き factory）。`<Type.Member …/>`・`<Type.member …/>` と書く所で `Type` が確定していれば縮められます。
...
- **memberwise（`<Type field=…/>`）は省略しません**。`<. field=…/>` は `.` の直後がフィールドで読みづらいため、型名を残します（leading-dot はメンバ名の前の型を省く形に限る）。
