...
`impl` の**主語は型でもトレイトでもよい**：`impl Type { … }` は型に、`impl Trait { … }` はそのトレイトの全準拠型に（提供メソッド）、`impl B as A { … }` はトレイト `B` の全準拠型を別トレイト `A` へ準拠させる（トレイト間準拠）。**トレイト自身の提供メソッドはベアな `impl Trait`（トレイトのモジュール内）に書きます**（→ [トレイト](08-traits.md)）。拡張内の `impl Trait` は**第三者がトレイトに足す追加メソッド**、`impl B as A` は**トレイト間準拠**で、いずれも `#Ext` で opt-in します（ベアの `impl B as A` は不可）。1 つの拡張に複数の `impl` を束ねられ（型・トレイト混在可）、**拡張の名前からは中身を推定できません**（後述の à la carte 適用）。
...
`#` は「**メソッド源の選択子**」です。`a#X.foo()` は「**X が提供する `foo`**」を呼びます ── X は**拡張**でも**トレイト**でもよく、どちらでも操作は同じ（"foo from X"）。`#Ext`（拡張ビュー）は型レベルにも現れますが（前節）、**`#P`（トレイト）は呼び出し位置の曖昧化専用**で、`A#P` のような型は作りません（トレイトを値の型にするのは [`any P`](08-traits.md#トレイトを値の型として使う存在型-any)）。

主用途は generic で**両方の境界に同名の提供メソッド**があるときの曖昧化です：
...
- `where T: P + Q`（P・Q が同名 `foo` を提供）は**満たせます**：両方をベアに持つ型は作れない（[提供メソッドの衝突](08-traits.md#提供メソッドの衝突同名はベアに同居できない)）が、片方を拡張に閉じた**増強型** `A#AWithQ`（P はベア・Q は #AWithQ）が P・Q の両方に準拠するので、`f(a#AWithQ)` で渡せる。
- 本体の `a#P.foo()` は**トレイト相対**なので、`a#AWithQ`（Q を拡張で持つ）でも `a#AWithP`（P を拡張で持つ）でも `a#P.foo()` は常に P の foo に解決＝**どちらの増強型を渡しても同じ関数で動く**（これが `Q#E` 型引数拡張を採らない理由：あれは `P + Q#E` と `P#E + Q` が別関数になり呼べる値が割れる）。
- これは「全メソッド呼び出しが一意に解決する」という Plew の原則を、**この曖昧化に限って**緩めたものです（`a#AWithQ.foo()` と `a#AWithP.foo()` は別メソッドだが、どちらも `f` に渡せる必要があるため）。曖昧化は常に `#` で明示するので解決は可視。
...
拡張内の `impl` は**トレイトを主語**にできます。用途は 2 つ：**第三者がトレイトに提供メソッドを足す**（トレイト所有者でなくても `extension` 経由で・`itertools` 相当）と、**トレイト間準拠**（あるトレイトの全準拠型を別トレイトへ）。トレイト**自身の**提供メソッドはベア `impl Trait`（→ [トレイト](08-traits.md)）で、ここに書くのは追加分です。
