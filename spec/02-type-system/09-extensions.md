...
### 既定拡張を剥がす（`#!` は導入元のトレイトに付ける）

トレイトの既定拡張がその型に合わない（同名の自前メソッドがある・複数トレイトの既定が衝突する）ときは、**その既定拡張を導入した準拠地点で剥がします** ── `impl A as Iterator#!IterExt {}`。既定拡張はトレイトに乗って来るので、剥がしも**トレイト側に統一**します（型本体 `defaultExtension #!Ext` や型パラメータ `T#!Ext` には書きません＝同義を複数記法にしない）。

```plew
impl A as Iterator#!IterExt {        // この準拠は IterExt を A のベア表面に載せない
    inout fn next() -> Optional[Item] { … }
}
```

- generic では境界に書きます ── `fn f[T](a: T) where T: Iterator#!IterExt`。
- 剥がしても**準拠関係そのものは不変**（A は Iterator のまま）で、他の場所の境界随伴にも影響しません。strip はそれを書いた地点（その準拠・その関数）のベア表面だけに効き、`a#IterExt.map()` で明示すればいつでも届きます。

### 共通規則

- **ベア解決への参加**：既定拡張（トレイト由来・型由来とも）のメソッドは、その型のベアなオーバーロード集合に加わる。既定でない拡張はベア解決に参加せず `#Ext` 必須。`value#!Ext` で個別に剥がせ、`value#Ext` は常に届く。
- **衝突＝型の定義地点でエラー**：型自身のメソッド・準拠トレイトの既定拡張・型が宣言した `defaultExtension` のいずれかが**同セレクタ・同シグネチャ**で衝突したら、その型の定義地点でコンパイルエラー（引数型が違えばオーバーロードとして共存）。解決は**既定拡張を導入したトレイト側で剥がす**：準拠地点なら `impl A as P#!Ext`、generic 境界なら `fn f[T](a: T) where T: A#!AExt + B`。剥がした側は `a#Ext.foo()` で明示すれば届く。`#!` は**既定拡張の持ち主であるトレイトに付く**＝`defaultExtension` 直後や型パラメータ `T#!Ext` には書かない。
