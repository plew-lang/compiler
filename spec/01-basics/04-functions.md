...
クロージャは外側のスコープの変数をキャプチャします。**値意味論なので、コピー可能は[コピー（スナップショット）でキャプチャ](03-values.md#値意味論value-semantics)**します。キャプチャしたコピーは**閉包環境に不変（`val`）として取り込まれます**：

- 外側の変数のその後の変更は閉包に伝わりません（キャプチャ時点のスナップショット）。
- **閉包はキャプチャしたコピーを書き換えられません**。元が `mut val` でも、取り込まれたコピーは `val` です。よってプレーンなキャプチャは**呼び出しをまたいで保持する可変状態にはなりません**（カウンタのように状態を持たせることはできない）。
- 閉包と外で**同じ可変状態を共有**したい・**呼び出しをまたいで状態を永続**させたいときは、[`Ref`](03-values.md#ref--weakref共有可変) を明示的にキャプチャします（共有可変の唯一の手段）。これが下の `make_counter` が `Ref` を使う理由です。共有・永続が常に `Ref` として**可視**になり、「コピーを変えたのに外が変わらない」という値意味論の取り違えを防ぎます。
...
- **コピー可能のキャプチャはコピー**（独立スナップショット・閉包内では `val`＝不変）。**`unique` 値はキャプチャできない**（閉包へ move する構文は当面なし）。共有可変・永続状態は `Ref` を copy-capture し、ARC が生存を保つ（`[weak self]`/`@escaping` 注釈は無い）。
