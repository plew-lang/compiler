...
`String` は**不変**な UTF-8 文字列です。一度作った中身は変わらず、加工（連結・変換・部分取り出し等）は新しい `String` を返します。参照の値渡し＋GC のもとで安全に共有でき、`spawn` の immutable キャプチャ（→ [非同期処理](../04-execution/14-concurrency.md)）とも噛み合うため、文字列は不変としています。
...
- **添字 `arr[i]` は `Index[I32] -> T`**（要素の値を返す。→ [型変換と演算子](../03-expressions/12-operators.md)）。`count` は O(1)。
...
`Dictionary[K, V]` は**キーから値への対応**を持つハッシュ表（`HashMap` / Swift の `Dictionary` 相当）。リテラル `[k: v]`／空辞書 `[:]` がこの型を作ります（可変アリティのため、配列リテラルと並んで「構築は JSX」原則の**本物の例外**です）。`Array` と同じくリテラル構文が参照する**言語アイテム**なので、`import` せず常にスコープにあります（→ [言語アイテム](../04-execution/15-modules.md#言語アイテムは常にスコープにあるimport-不要)）。
