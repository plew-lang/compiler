...
ラップ（2 の補数で巻き戻す）が必要なときは**明示**します ── ラップ演算子（`&+` など・名称暫定）か `wrapping_add` 等のメソッド（暫定）。0 除算 panic（→ [演算子](../03-expressions/12-operators.md#演算子システム)）と合わせ、整数演算は「静かに壊れない」を保ちます。

> Plew は意味論を変えるビルドモード（オーバーフロー検査の on/off 等）を持ちません。`debug`/`release` の差は最適化レベル（速度）だけで、観測可能な挙動は不変です（リリースでだけ落ちるバグを作らない＝開発しやすさ最優先）。
...
`String` は**不変**な UTF-8 文字列で、**CoW 値型**です。一度作った中身は変わらず、加工（連結・変換・部分取り出し等）は新しい `String` を返します。不変にしているのは UTF-8 妥当性の不変条件を保ちやすく `==` がバイト等価で済むためです（値意味論なので旧「参照＋可変＋共有」のエイリアス footgun は元々無く、Swift 流の可変 CoW String も将来は選べますが、当面は不変を維持します）。
...
`Array[T]` は**動的・伸長可能・連続**な **CoW 値型**のコレクション（`Vec<T>` / `List<T>` 相当）。コピーすると内部バッファを共有し、変更時にだけ複製します。要素は連続配置され、プリミティブは**ボックス化せずインライン格納**します（Java `int[]`・C# 配列モデル。`Array[U8]` や WASM ゼロコピーの前提）。**`unique` 要素は直接持てません**（generics は copyable 限定）ので [`Ref`](03-values.md#ref--weakref共有可変) 包み（`Array[Ref[T]]`）にします（→ [ジェネリクス](../02-type-system/06-generics.md#型引数の能力マーカーallow_unique--no_local)）。
...
`Dictionary[K, V]` は**キーから値への対応**を持つハッシュ表（`HashMap` / Swift の `Dictionary` 相当・`Array` 同様の **CoW 値型**）。リテラル `[k: v]`／空辞書 `[:]` がこの型を作ります（可変アリティのため、配列リテラルと並んで「構築は JSX」原則の**本物の例外**です）。`Array` と同じくリテラル構文が参照する**言語アイテム**なので、`import` せず常にスコープにあります（→ [言語アイテム](../04-execution/15-modules.md#言語アイテムは常にスコープにあるimport-不要)）。
