...
`Array[T]` は**動的・伸長可能・連続**な **CoW 値型**のコレクション（`Vec<T>` / `List<T>` 相当）。コピーすると内部バッファを共有し、変更時にだけ複製します。要素は連続配置され、プリミティブは**ボックス化せずインライン格納**します（Java `int[]`・C# 配列モデル。`Array[U8]` や WASM ゼロコピーの前提）。**`unique` 要素は直接持てません**（generics は コピー可能 限定）ので [`Ref`](03-values.md#ref--weakref共有可変) 包み（`Array[Ref[T]]`）にします（→ [ジェネリクス](../02-type-system/06-generics.md#型引数の能力マーカーallow_unique--no_local)）。
