...
文字列内で `{expression}` による変数展開が可能です（`Format` トレイトを実装した型で使えます）。`{x}` は `x.format(format: "")`、`{x:.2}` のようにコロン以降を書くとその指定文字列が `x.format(format: ".2")` へ渡されます。
...
- 補間に使う波括弧を**リテラルとして**書くには `\{` / `\}` でエスケープします（`\n` などと同じバックスラッシュ系）。
- 文字列リテラルは**生の改行を含められます**（複数行可。Rust と同様）。
