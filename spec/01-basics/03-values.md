...
- **循環**は参照カウントで回収されないので、断ち切りに **`WeakRef[T]`**（非所有・指す先が消え得る）を使います。循環が生じ得るのは `Ref` グラフだけで、その自動回収（Ref グラフ限定のサイクルコレクタ）は将来 additive に足せる方針です（→ [非同期処理とメモリ管理 § メモリ管理（ARC）](../04-execution/14-concurrency.md#メモリ管理arc)）。それまでは `WeakRef` ＋開発ビルドのリーク報告で運用します。**`WeakRef` は直接 deref できません（`w->x` は無い）** ── 指す先が生きている保証が無いからです。生存確認とアクセスは **`upgrade() -> Optional[Ref[T]]`** を通します（生きていれば強参照 `Ref`、消えていれば `None`＝可謬性が Optional で表に出る）：
