...
- このヘッダの `&&` は[値レベルの `&&`](07-operators.md)（Bool 専用の短絡）と**同じ「短絡する連言」概念ですが、脱糖は別物**です（ヘッダの `&&` は束縛節も連結できる条件チェーン文法）。
...
- `spawn` スレッド内の panic は**プロセス全体を停止**する。スレッド単位で扱いたい失敗は `Result` を返して `join()` 経由で受け取る（→ [非同期処理とメモリ管理](09-concurrency.md)）。
