...
1. **ビルドとは別コマンド・別ファイル出力。** Go の `go generate` や Dart の `buildRunner` のように、コンパイル（ビルド）とは独立した専用コマンドでメタプログラムを実行し、**生成結果を別ファイルの Plew ソースとして出力**します。コンパイル時にインラインで AST を展開する方式は採りません。生成物は通常の Plew ソースとして読め、追跡できます。
