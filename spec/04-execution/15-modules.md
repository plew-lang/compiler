...
- **外部公開ゲート（ルートの `export`）は通らない**。`/` は自パッケージ内の参照なので、外部到達を絞るルート `_.pw` の `export`（→ [公開モジュールと外部到達](#公開モジュールと外部到達)）とは無関係で、相対 `./`／`../` と同じく**自パッケージのどのモジュールにも届きます**。`/` と `./`／`../` の違いは「ルート絶対か・ファイル相対か」だけで、到達範囲は同じ。
...
- **`import` と `part` の使い分け（ベストプラクティス）**。既定は `import`：モジュールは非循環の層（DAG）に切る。**`part` を使うのは [無名 impl の配置](#無名-impl-の配置) のためだけ** ── 無名 `impl T`／トレイト提供メソッド `impl Trait` は型/トレイトの定義モジュールに置く必要があるので、その型の振る舞いを複数ファイルに分けたいとき `part` で同一モジュールへ綴じる。**`part` に書けるのは `impl` だけ**（自由関数・型・top-level `val` はモジュール root か別モジュールへ。[循環依存](#循環依存モジュールグラフは-dag)の節のとおり、相互再帰もこれで表現でき `part` に自由関数を置く理由はない）。「関連が近い」「ファイルが大きい」は `part` の理由にならない ── 層として切れるなら `import`、切れないなら振る舞いを `impl` 化して `part`。
...
パッケージの**公開面は、ルートモジュール `_.pw`（`src/_.pw`）が [`export`](#再エクスポート) で再公開したもの**だけです。**lib は構造的に 1 つ（ルート）**で、サブモジュールを外へ出すときはルートが `export ./Http` で再公開します（公開 API を組み立てる「バレル」がルート `_.pw`）。**公開パスは実ファイルに一致**し（別名を付けない＝`@A/Http` ↔ パッケージ内 `Http.pw`）、ルートが export していないモジュールは外部から到達できません。ソースは `lib/`・`bin/` のような分割をせず、すべて `src/` に置きます。

```plew
// src/_.pw（パッケージ A のルート＝公開の玄関）
export ./Http        // @A/Http を公開
export ./Cli         // @A/Cli を公開
```

```plew
import @A           // ✅ ルート（src/_.pw）
import @A/Http      // ✅ ルートが export 済 ↔ src/Http.pw（or src/Http/_.pw）
import @A/Internal  // ❌ ルートが export していない＝内部実装（到達不可）
```

- **lib は 1 つ（ルート `_.pw`）**。「複数の公開エントリを配列で列挙」する形（旧 `public` フィールド）は採りません ── データ構造として「複数 lib」を表現できないようにし、公開を**ルートの `export` に一本化**します（Rust の lib.rs モデル）。encapsulation は ファイル→モジュールが [`part`](#part--モジュールの分割)、モジュール→外部公開が [`export`](#エクスポート) の 2 層。
- **bin（実行可能）も同じ公開面**：ルートが export したモジュールが `main` を持てば公開 bin（利用者は `plew run @A/Cli`）、持たなければ lib モジュール（`import @A/Cli`）。**main の有無**で lib/bin が決まり（→ [実行エントリ](#実行エントリmain)）、専用フィールドは増やしません。
- **公開パスは実ファイル**（rename しない）。「公開ファイルを動かす＝公開 API を変える」を正直に出す Go 流の割り切り。別名公開（Node の `exports` 流）は将来 additive。
- **任意深さ可**：ルートが `export ./Http`、`Http` が `export ./Server` すれば `@A/Http/Server` に到達します。各段の `export` が公開を 1 段ずつ開ける（**既定は非公開**＝export しない限り内部実装）。
- **自前（`/`・`./`／`../`）とパッケージ `@…` は解決規則が同一**（`_.pw`＝ルート・`/`＝サブ）。違いは「外部パッケージのサブモジュールはルートの `export` ゲートを通る」点だけ。自パッケージ内の絶対パス `/` はこのゲートの内側なので縛られない（→ [ルート起点の絶対パス](#ルート起点の絶対パス)）。
