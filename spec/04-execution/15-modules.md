...
- **循環が要るなら、それは「1 つのモジュール」のサイン**。相互再帰などで本当に切り離せない複数ファイルは、**論理的には 1 つのモジュール**です（JPMS の言う「循環に関わるモジュールはどのみち 1 つのモジュール」）。その場合は別モジュールに分けず、[`part`](#part--モジュールの分割) で 1 モジュールに綴じてください（モジュール内＝part ツリー内は参照の循環自由）。役割分担は **`import`＝非循環の層境界**（別モジュールを参照）、**`part`＝1 つの論理モジュールの複数ファイル分割**。
...
- **`import` との使い分け（ベストプラクティス）**。既定は `import`（別モジュールとして非循環の層に切る）。`part` を選ぶのは次のどちらかが成り立つときだけ ── ①[無名 impl の配置](#無名-impl-の配置)が型/トレイトの定義モジュールを要求する、②相互再帰などで [DAG に切れない](#循環依存モジュールグラフは-dag)（＝論理的に 1 モジュール）。単に「関連が近い」「ファイルが大きい」だけでは `part` の理由にならず、層として切れるなら `import`。**循環が必要になったら、それが `part` のサイン**。
