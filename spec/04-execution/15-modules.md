...
- 数値型（`I8`…`U64`・`F32`/`F64`）・`Bool`・`String`・`Array`・`Optional`・`Result`・レンジ 2 型（`HalfOpenRange`/`ClosedRange`）・`Promise`・スレッドハンドル
- 演算子・変換・反復のトレイト（`Add`/`Eq`/`Ord`/`From`/`Iterator`/`Step` など）

これらは `1`・`"x"`・`[1, 2]`・`a..<b`・`x?`・`try`・`a + b`・`for` といった**構文の脱糖先**で、無ければ言語が成立しません。**固定された小さな一覧**（正確な一覧は別途定める）で、**ここに無いものはすべて通常のライブラリ＝明示 `import` が要ります**（`Random`・`HashMap`・`print` など。構文が参照しない＝言語アイテムではない）。判定基準は「構文が参照するか」で sharp です。
