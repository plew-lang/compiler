...
> **⚠️ この章は地金転換（値意味論+CoW+ARC+opt-in所有権）を反映済み。** 旧版は GC・「spawn キャプチャ全 immutable」・「データ競合あり得る（UB）」を前提にしていた。新モデルは **ARC** ＋ **借用は境界を越えない** ＋ **spawn は値の送信のみ** で **実質 race-free**。経緯は CLAUDE.md 冒頭「進行中の地金転換」節 / design-decisions の地金転換節を参照。
...
- **ARC で管理**：tracing GC ではなく参照カウント。スコープを抜けて最後の所有者／`Ref` が消えると**即座に**解放され、`deinit` が走る（決定的破棄＝ファイル・ソケット等の資源解放に使える）。
- **循環は `WeakRef` で**：参照カウントは循環を回収しないので、親子の逆リンク等は `WeakRef` にする（旧 GC の「循環ノーケア」は無い＝意識的なトレードオフ）。
...
新モデルは旧モデル（「データ競合あり得る・UB」）から**格上げ**されます：
