...
- **キャプチャは全て immutable（例外なし）**：spawn ブロックが外側からキャプチャした値は、元が `mut` でも spawn 内では不変。チャネルの `Sender` も immutable にキャプチャされる ── 例外ではない。**この保証は「spawn 内から spawn 外の状態を書き換えられない」こと**で、その帰結として **spawn 外のコードはロックを考えなくてよい**（これが唯一の保証で、「データ競合が起きない」保証ではない。下記「メモリ管理と並行安全性」）。
...
Plew はガベージコレクションを採用しており、基本的にメモリ管理を気にする必要はありません。シングルプロセス・シングルスレッドで動く限り、競合は原理的に存在せず安全です。

`spawn` のときだけ複数スレッドが同じ GC ヒープを触り得ます。Plew はここで **Rust 流の厳密な競合自由（data-race freedom）は提供しません**（できません）。提供するのは次の最小保証だけです。

- **spawn 書き込み隔離**：キャプチャは spawn 内で不変なので、spawn 内から spawn 外の状態を書き換えられない。よって **spawn 外のコードはロックを一切考えなくてよい**。これが唯一の保証です（「データ競合が起きない」保証ではありません）。
- **可視性は未規定**：spawn 内のキャプチャに対し、spawn 外での変更が spawn 内の read に反映されるかは**実装依存**（native は共有・WASM はコピーになり得る）。spawn 内 read が外の変更で揺れても正しさに影響しない使い方を前提とします。

**データ競合 ── spawn 外で可変な状態を spawn 間で共有し、未同期で read/write すること ── はプログラマの誤りで、未定義動作になり得ます**。表現次第（将来の `Slice`・存在型 `any P`・enum などの「裂け」）では**クラッシュやメモリ破壊**もあり得ます。ただし **idiomatic な並行コードはそもそも競合しません** ── キャプチャ全 immutable ＋「共有ではなく送信」（[チャネル](#チャネル)）。可変状態を spawn 間で共有するのは非推奨で、値のやり取りはチャネルで行ってください。

> **理想は「競合してもクラッシュはあり得るが未定義動作（メモリ破壊）は起きない」**（Go の `concurrent map` 検出のような検出即クラッシュ）ですが、これは恒久的に全複合型を参照表現に縛る等のコストを伴うため、実装・ランタイムコスト次第で各機能の実装時に判断します。現時点では保証しません。
