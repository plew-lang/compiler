...
if sh ./test-mid-reference-write.sh; then
    :
else
...
if sh ./test-mid-self-layout.sh; then
    :
else
    fail=$((fail + 1)); failed="$failed mid-self-layout"
fi
if sh ./test-mid-enum-owner.sh; then
    :
else
...
if sh ./test-mid-value-match.sh; then
    :
else
...
if sh ./test-mid-panic.sh; then
    :
else
    fail=$((fail + 1)); failed="$failed mid-panic"
fi
if sh ./test-mid-while-true-return.sh; then
    :
else
...
if python3 ./test-closure-environment-mid.py; then
    :
else
