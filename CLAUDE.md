...
- 1 ファイル 1 モジュール（`.pw`）・`part` 分割・ディレクトリは `_.pw`。**全定義はモジュールのトップレベル＝モジュールスコープ（グローバル名前空間なし・他モジュールは `import` 必須）**だが「グローバル関数/変数が無い」のではない：型に属さないトップレベル関数、トップレベル変数（`val` 定数・`mut val` 可変）は在る。static は別概念ではなく `assoc fn`/`assoc val`（名前違いの同等機能・翻意済み）→ spec/15,07。
