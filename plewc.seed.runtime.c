...
void plew_rawbuf_set_count(void* p, long long n);
...
char* plew_f64_to_str(double v){ char buf[32]; int n=snprintf(buf,32,"%g",v); char* b=plew_str_buf(n); memcpy(b,buf,(size_t)n); b[n]=0; plew_rawbuf_set_count(b,(long long)n); return b; }
...
void plew_inout_overlap(void* a, void* b){ if(a&&a==b){ fputs("panic: overlapping inout: two inout arguments of one call are the same place\n",stderr); exit(1); } }
...
static char* plew_readPath(const char* path){ FILE* f=fopen(path,"rb"); if(!f) return plew_str_empty(); fseek(f,0,SEEK_END); long sz=ftell(f); fseek(f,0,SEEK_SET); char* buf=plew_str_buf((long long)sz); size_t n=fread(buf,1,(size_t)sz,f); fclose(f); buf[n]=0; plew_rawbuf_set_count(buf,(long long)n); return buf; }
char* plew_readFile(char* d, long long n){ char* p=(char*)malloc((size_t)n+1); if(n) memcpy(p,d,(size_t)n); p[n]=0; char* r=plew_readPath(p); free(p); return r; }
