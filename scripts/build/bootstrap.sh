...
# regenerate the seed:
#   ./scripts/build/bootstrap.sh --reseed   (commit the updated seed files + plewc).
#
...
trace() {
    stage=$1
    shift
    python3 ./scripts/support/trace-command.py "$TRACE_DIR/$stage.log" -- "$@"
}
...
if cmp -s "$SEED_LL" plewc.check.ll && cmp -s "$SEED_RT" plewc.check.runtime.c; then
    ...
    echo "    run ./scripts/build/bootstrap.sh --reseed and commit the seed files + plewc." >&2
    exit 1
fi
