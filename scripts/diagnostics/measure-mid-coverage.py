...
Reject suites remain the responsibility of tests/harness/test.sh/tests/harness/test-gen.sh/tests/harness/test-deps.sh.
"""
import argparse
...
def main():
    ...
    if args.sources:
        ...
        # Match tests/harness/test-gen.sh: prepare generated parts before observing the app.
        ...
        for source in sorted(root.glob("tests/fixtures/gen/*/App.pw")):
            ...
            with (work / "harness.ll").open("wb") as stream:
                subprocess.run([sys.executable, str(root / "scripts/support/trace-command.py"),
    ...
    def observe(index, source, flags):
        ...
        directory.mkdir()
        command = [sys.executable, str(root / "scripts/support/trace-command.py"),
