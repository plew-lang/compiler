...
- ✅ **再帰値型**（`struct A { child: Optional[A] }`＝field-level auto-boxing＋CoW ARC・観測挙動も解放も正しい・5000 ノード 0 leaks・**generic 再帰値型 `N[T]{next:Optional[N[T]]}` も box 化＝mono クラッシュ解消**〔markBoxedFields の generic guard 除去〕＋**構築も動作**〔boxed-field の `st.expectedTy` を env で ground〕）。残〔follow-up〕：**generic 再帰型の match-bound payload field access**〔`v.v`＝`exprStructIdx` が generic-instance bind を struct 認識せず clean error・非再帰/primitive payload は✅〕・終端なし循環（住人なし＝構築不能・将来 loud reject）。spec/03。
