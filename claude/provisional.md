...
- ✅ **可視性 `export`/`import with`/`pub impl`/`pub`/`pub(get)` 完全強制**（`moduleOf`＋`checkImports`／`checkUseVisibility`＋メンバ可視性）。✅ **名前空間 import `Io.print`**・✅ **path-level alias `import @Std/Io as IO`**・✅ **name/type/value alias `with { real as alias }`**・✅ **`_.pw` ディレクトリ解決**〔`import /Models`→`src/Models/_.pw`〕。**残・未実装**＝パス正当性厳密検査〔今は「ロード済み export 集合に在るか」近似〕。spec/15。
