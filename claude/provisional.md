...
- **メタプログラミング（`Derive`・コード生成）** → ✅ **M1＋A 実装済**（tag `metaprogramming-m1`／A フロントエンド統合）：`plewc --gen` モード＋共有 `@Std/Syntax`（レクサ＋値ツリー AST `DeclAst`/`TopItemAst`＋宣言/本体/トップレベルパーサ `parseProgramAst`/`parseItem`＋出力 `Src`）＋ローダ auto-part＋`plew-gen.sh`。マクロが対象宣言の実構造（名前/フィールド/型/可視性/enum バリアント/fn シグネチャ＋本体）を読んで生成し、`@[Name(label: …)]` ディレクティブ引数も `self` から読める。**A 完了でコンパイラもマクロも唯一の共有パーサを通る（真の 1 AST）**。**剥離**：(1) ディレクティブ引数のファクトリ可視性は上記「構築・factory」のとおりゲート (b) のみ強制（(a) 公開ファクトリ宣言は未チェック）。(2) `parseItem` は struct/enum/fn のみ＝**impl/trait へのマクロ付与は未対応**（値型 `ImplAst`/`TraitAst` は在るので `parseItem` 拡張で additive）。残るは M2 dogfood（Eq/Hash→Dictionary）。spec/16。
