...
- ✅ **ラベル付き無名レコード `(x: I32, y: I32)`**（型注釈/リテラル/`.field`/多値返却/フィールド順独立〔canonical ソート＝構造的同一性〕/関数跨ぎ同一性/record param/value-semantics コピー/struct・enum payload/合成 struct 機構〔`ensureRecordStruct`〕/**分解 `(val x, val y) = p`**/**punning `(x, y)`**/**for 分解 `for (val x, val y) in xs`**/**construction-field & call-arg context**〔dict/record/`()` literal が field/param 型から推論〕/for-over-array element 追跡）。**残**：(a) **method への record literal context**〔receiver 型解決が要る・free fn は✅〕・(b) **nested record literal**〔record-of-records の inner literal context〕・(c) **inline `()` の generic field 構築**〔`<Result.Ok value=()/>`＝generic owner で field 型が param ゆえ grounding されず・変数経由で✅〕・(d) **canonical 名の full-type 厳密化**〔`recordTypeKey` は head＋arg-head のみ〕。spec/02。

## 型システム：ジェネリクス・トレイト・拡張

- ✅ **再帰値型**（`struct A { child: Optional[A] }`＝field-level auto-boxing＋CoW ARC・観測挙動も解放も正しい・5000 ノード 0 leaks）。残〔minor〕：終端なし循環（住人なし＝構築不能・将来 loud reject）。spec/03。
