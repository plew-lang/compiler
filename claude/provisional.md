...
- ⚠ **可視性 `export`/`import with`/`pub impl`/`pub`/`pub(get)`**（`moduleOf`＋`checkImports`／`checkUseVisibility`＋メンバ可視性）。✅ **名前空間 import `Io.print`**・✅ **path-level alias `import @Std/Io as IO`**・✅ **name/type/value alias `with { real as alias }`**・✅ **`_.pw` ディレクトリ解決**〔`import /Models`→`src/Models/_.pw`〕。**剥離（spec 逸脱・要修正）**＝①**クロスモジュール型参照が未 gate**：`checkUseVisibility` は `Expr.Call`（自由関数）だけを gate し、**型/トレイト名参照は無 import で通る**（コード上 "Types stay ambient" の手抜き）。spec/15 は lang item 以外の型も import 必須なので、型参照 gate を追加すべき。②**循環 import 未検出**：Loader は dedup でロードするだけで `import` グラフの循環を検出しない。[spec/15 循環依存](../spec/04-execution/15-modules.md#循環依存モジュールグラフは-dag)で **DAG 必須＝循環はコンパイルエラー**と確定したので、import グラフの DFS 循環検出を追加すべき。③パス正当性厳密検査〔今は「ロード済み export 集合に在るか」近似〕。spec/15。
- ✅ **`part`／`import` パスルート**（`part ./Name`・`@Std/X` は binary 横 `std/X.pw`・`with { }` 選択・dir module・namespace import）。spec/15。
- ✅ **ambient `@Std/Core`**（演算子 witness/`Optional`/`Result`/`for`/range が import 不要＝起動時 force-load・`assert` は import gate 維持）・✅ lang-item 型の**再定義を loud reject**。
