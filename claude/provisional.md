...
- ✅ **S2 @Std I/O の実 .pw 化**：`write`/`writeByte`/`readStdin`/`readFile`/`readFileBytes`/`eprint`/`fileExists`（`@Std/Io`）・`argCount`/`argAt`/`exit`（`@Std/Process`）・`sleep`（`@Std/Async`）は実 `.pw` の **`extern "plew-intrinsic"` 宣言**になり、コンパイラの名前ハードコード intercept は撤去（呼びは `plew_<name>` へ・ランタイム symbol は camelCase 統一）。lang item 型は **`extern "plew-intrinsic" { struct I32 … struct String }`**（Core が宣言＝所有・表現は intrinsic のまま不変・コヒーレンス是正で `impl I32 as Format` 等が通る）。**`Format` トレイト＋全整数型の `impl … as Format` を純 Plew で実装済**（`stringFromBytes` intrinsic＋桁ロジック・I8-32→I64／U8-32→U64 へ lossless 委譲・`(x).format(format: "")` 実証）。境界機構は [design-decisions.md](design-decisions.md)「コアライブラリの境界」。**残（延期）：`print` の純 Plew 化（`print[T: Format]`）**＝generic free fn の monomorphization の **scan 時型復元が未 robust**で、print を generic にすると全 print 呼びがこれに当たる（instance 未発見→未置換 `print_value_T` を吐き clang が落ちる）。**ハードニングの進捗と残ギャップ（実験で確認・next の地図）**：✅ **closure 呼び結果の型復元**（`exprType` が fn 型ローカルの呼びを fn 戻り型へ・コミット済 `wrap(add(n:5))→wrap_x_I32`）。残ギャップ＝(1) **`arr.count`→U64**（`exprType` の Field に count 分岐＋kwSpan 用に "U64" を `kwSpan builtins` コメントへ・**この修正単体は自己ビルド OK**）。(2) **generic-enum の match バインド**（`scanAddArmBinds` が `Optional[I64]` のペイロード `v` を型パラメータ `T` のまま登録→`print(v)` が `_T`。codegen 同様 `genericEnumFieldTypeInfo` で置換が要る。**ただし素朴な置換版は Check.pw 周辺の既存呼びで "argument labels do not match" の自己ビルド破壊を起こした**＝scan が登録するバインド型を変えると `inferFnArgs` 経由で収集する generic instance 集合が変わり、その emission が label 解決を狂わせる疑い・要精査）。(3) **式中裸リテラル**（`print(5 & 3)`＝両被演算子が裸→文脈なしで曖昧。テスト側でサフィックス）。(4) **配置の副作用**：print を `@Std/Io` に置くと Io が `@Std/Core` を import＝コンパイラ自身のコンパイルへ Core を巻き込み、`extern struct I32` が読まれて I32 が「所有」扱いになり **reject テスト `impl_on_builtin`（ユーザの `impl I32 {}`）が通ってしまう**＝print の置き場 or Core 巻き込みの再考が要る。それまで print はビルトイン継続（型に応じた `%lld`/`%llu`）。同根の `String`/`Array` メソッド Plew 化（generic intrinsic 床）も同じハードニング待ち。`readFile`→`Result` 可謬化（前提は揃い済）。
...
- **`part`／`import` のパスルート** → 部分実装。`part ./Name`（兄弟ファイル）・**`part ./Sub/Name`（サブディレクトリ part パス＝`Sub/Name.pw` に解決・コンパイラ自身が `part ./Codegen/Expr` 等で依存）**・`import @Std/Io with { … }`・`@Std/Process with { … }` の `with { }` 選択形をパース＆enforce（バイト列連結→単一アリーナ/単一 C 出力・スコープ共有/名前空間なしは spec 通り）。`@Std/X` は `dirname(argv[0])/std/X.pw` 解決＝**バイナリ横の `std/`**（リポジトリでは `compiler/plewc` 起動ゆえ `compiler/std/X.pw`・Core.pw が実在）。バイナリと std が隣り合う限りディレクトリ移動に強く**現運用（self-host/テスト）は無問題**。残る課題は**配布時のみ**＝`plewc` を PATH 経由の bare 名で起動すると argv[0] にディレクトリ接頭辞が無く stdRoot が cwd 相対に外れる→インストール先 or バイナリ埋め込みで std を置く設計はそこで詰める。フラグで gate する import 名は **`@Std/Io` の `print` のみ**（誤モジュール import は無効＝`import @Std/Process with { print }` は reject）。他の @Std 関数は実 `.pw` の `extern` 宣言を名前解決し、未 import＝未解決呼びは `compileError`（"unknown function … import @Std/…"・libc シンボル衝突回避）。enforce は stderr `plewc: error: …`＋`exit(1)`。残：`_.pw` ディレクトリ・ネストした part（root の part のみ走査）・`../`/`/` ルートの完全対応・forest/循環検査・行番号。
