...
- ✅ **再帰値型**（`struct A { child: Optional[A] }`＝field-level auto-boxing＋CoW ARC・観測挙動も解放も正しい・5000 ノード 0 leaks・**generic 再帰値型 `N[T]{next:Optional[N[T]]}` も box 化＝mono クラッシュ解消**〔markBoxedFields の generic guard 除去〕）。残〔minor〕：**generic 再帰型の構築**は「not yet supported」clean error〔クラッシュではない・follow-up〕・終端なし循環（住人なし＝構築不能・将来 loud reject）。spec/03。
