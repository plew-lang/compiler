...
- **クロージャ／関数値** → ✅ **実装済（不変値キャプチャまで）**：関数型 `fn(...)->R` は一律 **fat closure `PlewClosure{fn,env,rc,drop}`**（C 関数ポインタ単体から移行・呼び出し ABI は `R fn(void* env, params...)`・bare 関数値はサンク `<sel>__thunk` 経由・呼び出し位置で `.fn` を具体署名へキャスト）。クロージャリテラル＝`__closure<id>(void* __env, ...)`・高階関数。**キャプチャ＝不変（`val`）の値キャプチャ**：スカラー（整数/Bool）・String・**array・Ref・plain struct**（env 構造体 `__closure_env<id>` に格納・body は `((__closure_env<id>*)__env)->name`・`val` 不変ゆえ値キャプチャは参照キャプチャと観測同一＝spec 準拠）。ヒープ所有キャプチャ（array/Ref/struct）は **env に retain（share）して格納**＝外側スコープ解放後も生存（`makeFirst` がエスケープ後も有効）。**未対応は loud reject**（silent な意味逸脱を出さない）：`mut val` キャプチャ（参照共有が要る）・`unique`/generic 実体化/enum/関数値のキャプチャ・**ネスト closure**。**hidden cost**＝capturing 閉包の env と retain したヒープは現状解放せず leak（double-free/UAF 無し・`plew_closure_share/release`＋env drop は B2b で配線）。メソッド値は spec で禁止（spec 通り未対応）。**次（B2b）**＝`mut val` 参照キャプチャ＝**エスケープ変数のヒープ化（ARC 箱）＋ env drop＋閉包ライフタイム/move 追跡**（Swift 流参照キャプチャ・`makeCounter`・`spawn { block }` 前提）。
