...
- **再帰値型 `struct A { child: Optional[A] }` / `enum B { P(child: B) Q }`** → ✅ **実装済（field-level auto-boxing・観測挙動は正しい・box の ARC 解放のみ未配線＝leak）**。循環を閉じるフィールドを `markBoxedFields`（`Comp.boxedFields`・field の `nameStart` キー）が検出＝**concrete struct/enum の到達可能性ウォーク**（`Array`/`RawBuffer`/`Ref`/`WeakRef` は cut・`Optional`/`Result` は透過してその引数を辿る・back-edge を閉じる**ユーザー型側フィールド**を box）。codegen＝**そのフィールドだけ `T*` ポインタ化**（`genStructDef`/`genEnumDef`）・構築は `genBoxCell`（`malloc`＋値コピー＋`curRetTy=fd.ty` で nested `<Optional.Some/>` を `Optional_Node` に推論）・読みは deref コピー（`Expr.Field`／match payload bind）・代入は**fresh box に repoint**（共有セルを in-place 変更しない）。**値意味論は box 越しでも保持**（`rec_value_semantics` で実証＝コピー後に片方を repoint しても他方不変）。`Optional`/`Result` 自体は box しない（hardcoded `.data.Some.v` 等を温存）＝box するのは Optional に入るユーザー側フィールド。`fieldType` は generic-inst フィールドの `ref` を載せるよう修正（field の match が `Optional_Node` を復元）。`structNeedsCopy`/`structNeedsRelease`/`emitStruct{Copy,Share,Release}Def`/`typeIsTransitivelyUnique` は**boxed フィールドを skip**（plain 共有ポインタゆえ managed copy/release 不要・かつ循環ウォークの発散を断つ）。テスト＝`rec_list`/`rec_struct_optional`/`rec_tree`/`rec_mutual`/`rec_value_semantics`。**残（hidden cost・additive）**：box セルの **ARC 解放未配線＝leak**（観測挙動は正しい・短命プロセスでは無害）→ 将来 share=retain/release=deref-free の CoW box へ。**終端なし循環**（`struct A { val a: A }` ＝enum 経由でない構造体だけの循環）は有限型だが**構築不能**（住人なし・現状はクラッシュせず通常コンパイル＝loud reject は将来）。spec/03「再帰する値型」。
