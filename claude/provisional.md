...
- **構築 `<E.V wrongfield=…/>` / `<S wrongfield=…/>` のフィールド名実在検査** → **未実装（小・部分的に捕捉）**。`makeFieldType` が存在しないフィールドに `scalarInfo()` を返すため、値が**裸リテラル**なら「integer literal has no type from context」で（誤導的だが）reject されるが、**非リテラル値だと silent に通って broken C** になり得る。match パターン側（上）と同型の穴で、`checkMakeFields` に同様のフィールド実在検査を足せば塞がる（受理の健全性）。spec/05。
