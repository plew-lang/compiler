...
- ✅ **scoped resolution（可視性 `export`/`import with`/`pub impl`/`pub`/`pub(get)`）**：名前解決をスコープ化（`nameVisibleFrom`＝同モジュール定義 or imported+exported）。**自由関数**＝`findFunc` が in-scope 候補のみ選び `checkUseVisibility` が診断、**型/トレイト**＝`checkTypeVisibility` が宣言サイト（func param/ret・struct/enum フィールド）の書かれた型名を gate（use-site module は包含 decl の `defOffset`・型注釈スパンは再 intern ゆえ）。ネストした generic 引数も `checkTypeArgs` が再帰 walk（`Optional[Widget]`・`Dictionary[U64, Widget]` の引数位置も網羅・配列だけ head に要素を持つ特例ではない）。同 decl の型パラメータ名は skip（`FilterIter[I,P,E]` の P 等）。✅ **名前空間 import `Io.print`**・✅ **alias（path/name/type/value）**・✅ **`_.pw` ディレクトリ解決**。✅ **循環 import 検出**＝`detectImportCycles`（import→定義モジュール辺を DFS color・back-edge を reject・[spec/15 循環依存](../spec/04-execution/15-modules.md#循環依存モジュールグラフは-dag)）。**残〔小〕**＝①宣言サイト（free/impl 関数の param/ret・struct/enum フィールド・ネスト generic 引数）は網羅だが、**未 walk のサイト**＝ローカル `val x: T` 注釈／newtype の underlying 型／trait の要求シグネチャ・supertrait・assoc 型境界（いずれも局所 or 稀で、import 漏れは別サイトで露見しやすい）。②パス正当性厳密検査〔「ロード済み export 集合に在るか」近似〕。spec/15。
