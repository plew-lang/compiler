...
- ✅ **再帰値型**（`struct A { child: Optional[A] }`＝field-level auto-boxing＋CoW ARC・観測挙動も解放も正しい・5000 ノード 0 leaks）。✅ **generic 再帰値型 `Node[T]{next:Optional[Node[T]]}`・`enum Tree[T]{Branch(l:Tree[T],r:Tree[T])}` も完全動作**〔構築・match・field access end-to-end・test recursive_generic：linked list/tree〕＝5 link（markBoxedFields generic guard 除去／boxed-field 構築型 ground／`registerGenStructBodies` で instance layout box／`bindMatchPayload` の structIdx を `genStructSlot`／boxed-field read を base env で ground）。残〔follow-up・非クラッシュ〕：**相互再帰 generic**〔`A[T]`↔`B[T]`＝「construction not yet supported」clean error〕・**String payload の再帰 generic**〔I64 は✅・String は runtime 無出力 miscompile 疑い〕・終端なし循環（住人なし＝構築不能・将来 loud reject）。spec/03。
