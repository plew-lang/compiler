...
| メソッドのオーバーロード | 同名を引数型で区別（セレクタ＝名前＋ラベル、形一致・具体位置で区別・境界非関与） | 文法は impl 内に同名 `method` を複数書けるが、規則は**意味論層で検査**＝**仕様が先行** |
...
| レンジ構文 | `a..<b`(半開)/`a..=b`(閉)・素の `..` なし・2 型 `HalfOpenRange`/`ClosedRange` への**固定 JSX 糖衣**（非トレイト）・要素 `Ord`／反復は `Step` 条件付き（整数のみ） | 現行文法に範囲リテラルなし（要確認）＝**仕様が先行**（型・糖衣・トレイト境界は意味論層） |
