...
**構文の正典は `SPEC.md`／`spec/*.md`**。本書は stage0 の**手書き再帰下降パーサが支えるべき構文の一覧**を、各 spec 章への索引としてまとめる。旧 ANTLR 文法 `grammer/Plew.g4` は実装着手時に破棄した（構文解析は手書きへ移行）。`note/` は記法が時系列で変遷した一次資料なので、迷ったら spec を優先する。
...
> **3 列目「現行文法の状態」は破棄した `Plew.g4` の状態＝歴史的情報**。パーサ実装では 1・2 列目（構文と spec リンク）を使う。実装が進んだら stage0 パーサの対応状況へ置き換えてよい。
