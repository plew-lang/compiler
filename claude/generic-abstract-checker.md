...
- **残＝trait provided-method 本体の未検査（次の本命）**：`pub impl Trait { fn provided() {...} }` の本体は `verifyProgram`〔`methodRecvIsTrait` で skip〕も `checkGenericBodies`〔typeParams=0 で skip〕も通らず未検査＝`fn bad()->I64 { return "s" }` を accept。**Self を abstract 型として扱い**〔capability＝trait の reqs＋supertrait〕本体を検査する必要〔型パラメータの Self 版〕。checkGenericBodies を provided-method も walk させ、Self を type-param 同様に扱う方向。
