...
**✅ part=impl-only はコンパイラ全体で達成済**（commit `a84c9d4`・不動点緑 run=307）。`Codegen/`・`Parser/`・`Backend/Llvm/` 配下の全 part ファイルは `impl` ブロックのみを含む。Frontend は `impl Comp`、Backend は `impl LlvmCtx`。自由関数・型・extern は各モジュール root（`Frontend.pw`／`Backend.pw`）へ集約。以下は到達経緯（レシピと罠）。
