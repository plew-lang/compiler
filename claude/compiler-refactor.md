...
進捗：keystone（Comp を Frontend へ）済。**Frontend の part は全て impl-only 達成**（commit `b851108`・不動点緑）＝(1) Codegen/Parser 配下の c-first 自由関数を全て methodize（Module/ExprType/Lower\*/Check\*/Mono\*/Resolve\*/Verify\*/Emit/Ops/Stmt/Parser/Decl…＋ Loader の `parseAndLowerFile`）、(2) 残る非 c 純ヘルパ 29 個を `tools/relocate_free.py` で module root（Frontend.pw）へ verbatim 退避（同一モジュール scope ＝ call site 不変）、(3) `runFrontend` を root へ移し空いた `Codegen/Driver.pw` を削除（part 行も除去）。退避した自由型＝`LetEff`/`ConstInt`/`IntTy`/`ScalarCond`（Frontend.pw root）。**残＝Backend の part を `impl LlvmCtx` へ**（`st: inout LlvmCtx` 受け手・303 自由関数）＝part=impl-only の最終形。1 ファイルずつ機械的に：
...
**Backend track（`impl LlvmCtx`）の現状＝ツールは在るが backend codegen の穴で保留（根本未特定）**：`tools/methodize_st.py` が backend の `(c: inout Comp, st: inout LlvmCtx, …)`／`(st: inout LlvmCtx, …)` 関数を `impl LlvmCtx`（self=st・c は引数のまま）へメソッド化（Frontend 用 methodize.py の st 受け手版・mixed 対応・c-only と純ヘルパは free 据え置き／c-only を `impl Comp` に出来ないのはコヒーレンス＝Comp の定義モジュールは Frontend）。33 ファイルをほぼ全変換しビルドすると **`self.b` 等の self フィールドアクセスが一部で `field access only on a registered struct` で落ちる**。**症状（debug 済）**：メソッド本体直下の self は OK（`recvSidx` は 81 メソッドで valid 確認）が、落ちる箇所では `findLocal("self")` が **li=2（nLocals=3 のフレーム）** を返し その local の `structIdx` が invalid。**根本は未特定**：当初クロージャ捕捉を疑ったが compiler/src・std に**クロージャリテラルは皆無**なので捕捉ではない。self が高 index に再束縛される別経路（generic 実体化・thunk・某 desugar フレーム？）の self 型回復漏れと思われる＝**次に着手するならまず落ちる関数を1個特定**（debug 版 backend で `st.curFn` 名/関数 index を print）してから。script では直らない backend codegen 案件。ひとまず未コミット分は revert 済（main は Frontend methodize 完了の緑状態）。1 ファイルずつ機械的に：
