...
進捗：keystone（Comp を Frontend へ）済。**Codegen/Parser 配下の c-first 自由関数はほぼ全て methodize 済**（Module/ExprType/Lower\*/Check\*/Mono\*/Resolve\*/Verify\*/Emit/Ops/Stmt/Parser/Decl…＝混在ファイルは c-first だけメソッド化し非 c 自由関数は据え置き）。退避した自由型＝`LetEff`/`ConstInt`/`IntTy`/`ScalarCond`（Frontend.pw root）。残＝Backend を `impl LlvmCtx` へ（`st: inout LlvmCtx` 受け手）・part 内に残る非 c 自由関数の module root 退避（part=impl-only の最終形）。1 ファイルずつ機械的に：

**Backend track（`impl LlvmCtx`）の現状＝ツールは在るが backend codegen の穴で保留**：`tools/methodize_st.py` が backend の `(c: inout Comp, st: inout LlvmCtx, …)`／`(st: inout LlvmCtx, …)` 関数を `impl LlvmCtx`（self=st・c は引数のまま）へメソッド化（Frontend 用 methodize.py の st 受け手版・mixed 対応・c-only と純ヘルパは free 据え置き）。33 ファイル中ほぼ全てを変換しビルドすると **`self.b` 等の self フィールドアクセスが一部で `field access only on a registered struct` で落ちる**。診断＝**self を捕捉するクロージャ**の capture binding が self の構造体型を回復できない（self は型注釈スパンを持たない＝`Closures.pw` の capture push が `localStructIdxFor(cap.tyStart,cap.tyLen)` に空スパンを渡す／boxed capture は `structIdx=structs.count` 即ハードコード＝337 行）。メソッド本体直下の `self.b` は OK（recvSidx は 81 メソッドで valid 確認済）＝**問題はクロージャ捕捉時の self 型回復のみ**。これは backend codegen の修正案件（capture entry に self の受け手型スパン／structIdx を持たせる）であり script では直らない。Frontend は self 捕捉クロージャが無く露見しなかった。**Backend methodize は本修正を入れてから再開**。ひとまず未コミット分は revert 済（main は Frontend methodize 完了の緑状態）。1 ファイルずつ機械的に：
