...
- `impl MyStruct as Add[Rhs] { … }` = **無名（デフォルト）実装**。`s + t` は `t` の型で対応する `Add[…]` にバインド（オーバーロード解決）。
- `extension MyExt { impl MyStruct as Add { … } }` = **名前付き実装**。`s#MyExt + t` のように呼び出し位置で明示指定された時のみ発動。
- **拡張で追加できるもの**: メソッド / 可変メソッド、関連値（`assoc val`）、型エイリアス、factory、`via` フィールドエイリアス、トレイト実装。1 つの `extension` 内で複数の型を拡張可。
