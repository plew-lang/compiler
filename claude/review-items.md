...
- **（決定済 2026-06-04）struct フィールド宣言の区切りは改行のみ・カンマ不採用**：spec/05 通り改行区切りで確定（カンマは friendlier だが spec に無いので採らない）。現状の F7（カンマ区切り struct フィールドを loud-reject）が確定挙動＝**コード変更不要**。
  - **注意（矛盾ではない）**：enum の variant フィールドは `(label: Type)` の**レコード（ラベル付きタプル）と同形**で、レコードはカンマ区切り（`(x: I32, y: I32)`）。よって「struct フィールド宣言＝改行／variant フィールド＝カンマ」は意図的な区別で正しい（parseEnum が variant フィールドでカンマを受けるのは正常）。
