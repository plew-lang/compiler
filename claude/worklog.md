...
- **Bug10/11/12 ＝ネスト generic 単相化クラスタ（深い・要集中作業）**：調査で**共通の根**が判明＝**array の TypeInfo が compound 要素 ref を落とす**（`typeInfoOfRef` の array 枝が `typeInfoOfName(el.name, isArray:true)` を返し `ti.ref=0`・要素の型 args を捨てる）。結果、`Array[Pair[K,V]]`〔generic struct の field〕が **多数の emit サイト**で head-only mangle `Array_Pair`（args 欠落）を吐く＝壊れた C。確認したサイト＝(a) array runtime/typedef〔template `Array_Pair_K_V` は emit すべきでない・`skipArrayElem`/`emitArrayMethods` に `tyRefIsGround` 判定追加で skip〕(b) mono struct の ARC field action〔`emitMonoFieldAction`〕(c) JSX 構築の field share〔`genCopyValue`〕…他にもメソッド本体等。**部分修正は試したが多サイトで piecemeal は不適**＝main は一旦 revert（green 維持）。**正しい直し**＝`typeInfoOfRef` の array 枝が**要素 ref を保持**（`ti.ref=要素ref`）し、`wPA`/`genCopyValue`/field-action 等の array C 名 emit を **`emitMangle(要素ref)`〔env 越しに型 args 解決・`Array_Pair_I64_I64`〕で一元化**。中核 codegen 規約の変更ゆえ fixpoint 注意＝**集中作業**。Bug11（generic コンストラクタ戻り型推論）・Bug12（field 由来 `Array_U64_first` 本体の `Array_T_get`）も同じ単相化系統。**Dictionary を実テストにする dict-wip で一括が筋**（main の Bug1-9 を dict-wip へ取り込み→クラスタ修正→Dictionary 完成→main へ）。状態＝根本特定済・着手は集中ブロックで。
