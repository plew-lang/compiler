...
7. 🔨 **次の本丸**：
   ...
   - ⏭ 次：**plewc.pw を育てて self-host fixpoint へ**。残り＝①`struct`/JSX 構築/`.field`→②`enum`+`match`→③`Array`/添字/`append`/`String.bytes`→④型注釈を実際に C 型へ反映（int64_t 決め打ちをやめる・`U8`/`U64`/`Bool`/`String`/`Array[T]`）→⑤`for`/`break`/`continue`→⑥**`inout` 引数**（plewc.pw 自身が多用＝self-host 必須）。自分自身を食えるサブセットに達したら **`plewc < plewc.pw`** で self-host。**ファイル I/O**（自分を読む）と名前解決（線形スキャン or `Dictionary`）はその過程で。AST 再帰は arena+index（U64 index）で回避済。
