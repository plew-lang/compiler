...
- **グリーン**：`./test.sh`（run271/panic13/reject270/part6/fail=0＋不動点）／`./test-gen.sh`（gen10/reject1）。
...
- **条件チェーン＋`guard` 文**（spec/11「条件チェーン（束縛つき条件）」＝match ガード revert の代替で**本来 spec にある絞り込み機構**）＝中〜大。**`if`/`while`/`guard` の条件が `&&` 連結の節列**で、各節は ①Bool 式 ②**反証可能束縛 `PATTERN = expr`**〔Swift `if let`/`while let`/`guard let` 相当〕。束縛変数は後続節＋本体で有効。`guard` は**不成立時**に本体（発散必須）を実行し、束縛は guard 後も有効。**実装の難所**＝(a) 反証可能束縛の脱糖〔`if P=e && c { then } else { els }` → `match e { P => { if c { then } else { els } }  _ => { els } }`・**`else` が失敗点ごとに重複**＝give-block か共有 else helper で重複回避が要る〕、(b) **guard の binding-escape**〔束縛が guard 文の後の enclosing scope に生きる＝match 脱糖では body スコープに閉じるので、guard だけ「マッチ成功部の束縛を外へ」展開が要る〕、(c) Bool 専用 `guard c { div }` は `if !c { div }` で簡単だが低価値・本命は反証可能束縛。**段取り案**＝Bool guard → if 反証束縛 → while 反証束縛 → guard 反証束縛（escape）。`KwGuard` トークン要確認（contextual か）。
- **一般 Chain トレイト `?.`**（現 Optional 具体）・**`Pow`/`**`**（float 後・spec/12＝`assoc fn pow(base: Self, exp: Exp) -> Output`・`pow(base: x, exp: 2)`）・**match ネストパターン**（`Some(Point{x,y})`＝再帰 destructure・PatternAst.binds を nested pattern 化＋脱糖）・Bool dict キー（低価値）＝各小〜中。
