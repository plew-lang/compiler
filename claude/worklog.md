...
4. 🔄 **ARC ＋ `Ref`/`WeakRef`**：✅ 基本 `Ref[T]`。✅ **スコープフレーム基盤**＝`genBlock`/`for`/`match` 入口で `scopeMark`→出口で `scopeExit`（`emitScopeDrops`〔今は no-op〕＋`popLocals`）。名前解決を真の lexical scope に厳格化（回帰テスト `scope_shadow`）。✅ **ASan ハーネス**（`ASAN=1 ./test.sh`・free 導入時の UAF/double-free を機械検証・leak 検出は free 後に `detect_leaks=1`）。
   ...
   - **残り leak（task 3・別件・hidden cost）**：**`Ref` 箱の refcount 化**（現状 `Ref[T]`＝C `T*` の malloc で leak）。要：箱に inline rc ヘッダ・bind で retain・scope 末 release・boxed T 再帰解放（型別 Ref release fn）。配列/struct と並列の作りだが**コンパイラ自身は Ref 非使用ゆえ ASan 自己コンパイルで検証されない**（ref テストのみ）＝別途集中して。mono struct/enum の share/release も未（mono 局所は leak・安全）。値型の leak（配列/struct/入れ子/早期exit）は全解消・ASan 実証済み。
