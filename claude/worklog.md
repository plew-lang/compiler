...
**進行中の大物＝コンパイラ構造リファクタ**（83 フィールドの `Comp` god-object 解体＋巨大ファイル分割＋モジュール化）＝[compiler-refactor.md](compiler-refactor.md)（診断・到達点・マイルストーン M0-M4・未検証リスク）。引数名/ラベルは [spec/04 規約](../spec/01-basics/04-functions.md#引数名とラベルの指針規約) に揃える。**M0〜M3＋M5 完了**（`Comp` 11 フィールド・巨大ファイル全分割・scoped resolution）。残り M4 に着手。**M4 の主目的は part 撲滅＝モジュール構造を spec/15 ベストプラクティス（既定 import・part は相互再帰核/無名 impl のみ）に沿わせる**（91 part の単一モジュール解消）。型回復統一（typed IR）は当初 B として主目標化したが、part 撲滅には不要（Backend→Frontend の型回復呼びは一方向で循環せず）と判明し**後段の任意トラックへ退避**。進捗：**M4-1 完了＝IR をデータモジュール `Ir.pw` へ昇格**（旧 `Ast.pw`・初の cross-module 共有可変 `Comp`・tag `refactor-m4-1`）。予備作業として `findFunc` 解決を単一チョークポイント `resolvedCallee`（exprId キー永続 memo `TypeCache.callee`）へ集約済（13 site 中 12・tag `refactor-m4-b1`）＋`TypeInfo.viewExtId` 追加（typed IR stage1）。次＝**M4-2＝Backend↔Frontend の循環を断つ**（逆依存3関数 `enumIdxByName`/`findTraitIdxByName`/`methodInEntryModule` を Backend→Frontend/Ir へ戻す pure-move）→ Backend 独立モジュール化。
