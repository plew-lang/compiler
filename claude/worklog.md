...
- ✅ **ファクトリ公開ゲート (a)（spec/05・2026-06-12・done）**＝全 pub フィールドでも cross-module 構築は `pub impl Type { factory }` 明示要。実装＝**`MemberKind.Factory`**〔bare `factory` member・contextual keyword・lexer 変更不要〕＋`lowerImpl` が `im.isPub` のとき struct name id を **`c.pubFactories`** へ記録＋`checkConstructVis` が cross-module 構築〔代表フィールド offset の module ≠ defMod〕で pub factory 無しを reject。**ADD→reseed→USE の 2 段**で chicken-and-egg 解消〔stage1＝parser+lowering＋gate 無効化〔`if false`〕→reseed で種に factory 構文を焼く／stage2＝std〔`pub impl P/Lexer { factory }`〕追加＋gate 有効化→reseed〕。実際に cross-module 構築するのはコンパイラの `<P/>`/`<Lexer/>`〔Loader〕のみ＝この 2 型に factory 追加で self-host 緑。test reject factory_gate_cross_module〔`<Span/>` cross-module〕・run factory_gate_valid〔`<Lexer/>` cross-module OK〕。残（additive）＝`factory` の body 付き形・`optional`/`result factory`〔可謬構築〕・From/Chain/TryFrom の factory 構築要求。
...
- ✅ **use-after-move（2026-06-12・done）**＝`take(move u); take(move u)` / move 後の read を reject。verify に moved-index 集合 `c.verifyMovedIdx`〔c.locals index〕＝`move u` で mark〔operand を先に verify＝二重 move/read は Ident 枝で先に捕捉〕・Ident read で `verifyIsMoved`→reject・plain `u = e` 再代入で unmark・block scope 退出で `verifyDropMovedAbove(mark)`〔index 再利用の false-flag 防止〕。既存の conditional-move guard〔`localIndexByName < curBranchBase` を reject〕と組で linear 追跡が健全〔branch を跨ぐ move は元から禁止〕。コンパイラ source は `move` を実質使わない（ゼロ false-reject）。test reject use_after_move_double/use_after_move_read・run use_after_move_reassign_valid。残（spec 通り将来）＝partial/field move〔現状 field move 自体 unsupported〕・conditional move〔禁止のまま〕。spec/03。
