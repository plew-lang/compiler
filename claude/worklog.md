...
**Hash → Dictionary（M2 dogfood）はほぼ達成**：`Hash`/`Hasher` トレイト＋SipHash-1-3 Hasher＋`@[Hash]` derive dogfood、`Dictionary[K,V]` lang item（struct＋メソッド＋添字 `dict[k]`/`dict[k]=v`）まで land 済（詳細＝下記「Dictionary」）。**残＝`[k:v]`/`[:]` リテラルのみ**（再帰値型 boxing の罠で保留・下記）。次の本線は M2 仕上げ（リテラル）＋並行 additive（下記ロードマップ）。**M3＝パッケージ管理導入後に `@Std/Syntax` を in-tree から外部共有パッケージへ昇格**（コンパイラもマクロも同一版に依存＝Rust の 2 パーサ版違い問題回避）。
...
**🚧 残＝`[k:v]`/`[:]` リテラル（⚠ 再帰値型 boxing の罠で保留・撤回済）**：`ExprAst.DictLit(keys: Array[ExprAst], vals: Array[ExprAst])` を AST enum に足すと、**配列 of compound（`[<P x=1/>]`/`[[1,2]]`）が全部 segfault** する回帰が出た＝`lowerExpr(e: ExprAst)` の値渡しコピー（`ExprAst_copy`）が壊れ `match e` のタグ読みで segfault（コピー結果が garbage）。原因は **再帰値型 ExprAst に `Array[ExprAst]` を 2 本持つ新バリアントを足すと auto-boxing/copy 解析が乱れる**（`markBoxedFields`／value-tree copy 生成）。enum 末尾でも再現＝バリアント追加そのものがトリガ。リテラル単体（`["a":1]`/`[:]`/unsuffixed）は動いたが回帰のため全 revert。**再開方針**＝(a) `markBoxedFields`/再帰値型 copy が「`Array[ExprAst]` 2 本の新バリアント」で壊れる根因を先に潰す（最小再現＝`ExprAst` に `Foo(a: Array[ExprAst], b: Array[ExprAst])` を足すだけで `[<P x=1/>]` が落ちるはず・要確認）か、(b) 添字と同じく arena 新バリアント無しで codegen synthesize。**パース案**＝`Kind.LBracket`（ParseBody.pw）で `[` 後が `:]` なら空 dict／最初の式の後が `:` なら dict（`, k: v` 反復）／他は配列。当面リテラル無しでも実用可。
...
- **arena Expr/AST バリアント追加の罠**：①exhaustive な `match` 全箇所（Lower の `ExprAst`／genExpr・exprType・scanExprInsts 等の arena `Expr`）にケース追加が要る（コンパイル時に non-exhaustive で検出）。②**再帰値型へのバリアント追加は auto-boxing/copy を壊しうる**（上記「再帰値型 box」の罠）＝配列 of compound の copy で回帰確認。
