...
7. 🔨 **次の本丸**：
   ...
   - ⏭ 次：**plewc.pw を育てて self-host fixpoint へ**。順に＝①複数関数＋引数＋ユーザー関数呼び出し（Call 式・enum payload に `Array[U64]` を載せられるか要検証）＋戻り値型→②`struct`/JSX 構築/`.field`→③`enum`+`match`→④`Array`/添字/`append`/`String.bytes`→⑤型注釈を実際に C 型へ反映（int64_t 決め打ちをやめる）→⑥`for`/`break`/`continue`/`inout`。自分自身を食えるサブセットに達したら **`plewc < plewc.pw`** で self-host。**ファイル I/O**（自分を読む）と名前解決（線形スキャン or `Dictionary`）はその過程で。AST 再帰は arena+index（U64 index）で回避済。
