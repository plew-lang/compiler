...
- **拡張 `#Ext`（spec/09・実装中）**：**増分1 済**（2026-06-10）＝`extension Name { impl Type { fn } }` parse＋メソッドを `Func.extId` でタグ（bare 解決から除外）＋呼び出し `value#Ext.m()`（`Kind.Hash`＋arena `Expr.ExtMethod`＋`findExtMethod`）＝同名別拡張も `#A`/`#B` で区別・test `extension_basic`/reject `extension_bare_call`。**増分2 済**（2026-06-10）＝`defaultExtension #Ext`（struct 本体宣言→拡張メソッドがベア面に載る・`Comp.defaultExts`＋`findMethod` の extId skip 緩和・`pub impl` 必須・test `extension_default`）。**増分3 済**（2026-06-10）＝`#P` トレイト源選択（`a#P.foo()`＝提供メソッド衝突の disambiguation・`Func.srcTraitId` タグ＋`findExtMethod` が extId/srcTraitId 両対応・併存許容で曖昧ベア呼びだけ use-site reject・test `trait_source_select`）。**サブ増分3a 済**（2026-06-10）＝型レベル `Type#Ext`＝型位置 `Person#Ext`＋値再ビュー `v#Ext`（arena `Expr.ExtView`・`TypeRef.viewExtId`）＋viewed 値への bare メソッドが拡張優先解決（`exprViewExtId`→`findExtMethodById`・param/let が viewExtId を locTypeRef に伝播）・view shadow も動く・test `extension_type_view`。**サブ増分3b 済**（2026-06-10）＝不変リファインメント強制を **call 引数・annotated let・return** の3面で（`genArgValue`/`Stmt.Let`/`Stmt.Return`＝viewExtId 突き合わせ・双方向 reject・reject `extension_view_mismatch`/`extension_view_let_mismatch`）＋**view 伝播**（viewed binding/param/**viewed return**＝`exprViewExtId` が Call retTy view を解決し誤 reject 回避）。**self-view/strip/chain 済**（2026-06-10）＝ext メソッド self は `Recv#Ext` ビュー（`self.m()` 再帰・sibling 可）・`#!Ext` strip super（`ExtView.strip`・`self#!Ext.greet()` で元メソッド）・スタック `value#A#B`（`collectViewChain`＋各 view 試行・同名2 view 曖昧 reject）・test `extension_super_chain`/reject `extension_stacked_conflict`。型レベル chained `Type#A#B` は単一 viewExtId ゆえ未（値レベルは可）。**代入 view チェック＋bare 誤呼び診断 済**（2026-06-10・`Stmt.Assign` view 突き合わせ・`extMethodNameExists` 案内・reject `extension_view_assign_mismatch`）＝view 強制は call/let/return/代入の4面。**view-aware オーバーロード解決 済**（2026-06-10・`findFuncViewAware`＝`f(a:P)`/`f(a:P#Ext)` を引数 view で選び分け・**罠**＝front-end paramsTypesMatch は局所 struct 引数の型を見れずラベルのみ照合ゆえ backend で view 精緻化・test `extension_overload_view`）。コンテナ不変性は exotic ゆえ意図的保留。**トレイト主語の拡張・外部コヒーレンス・defaultExtension 衝突検査 済**（2026-06-10）＝`impl Trait`/`impl B as A`（`instantiateProvidedMethods` が extId 保持・test `extension_trait_subject`）・外部型コヒーレンス（別 part の型を拡張＝view 機構が orphan rule を満たす・検証済）・`checkDefaultExtCollisions`（reject `extension_default_collision`）。**🎉 拡張システムは struct レシーバで完備**（spec/09 の主要機能を全実装）。**残（拡張タスク外＝前提機能）**＝enum/primitive/generic の ext レシーバ＋enum defaultExtension は**通常の enum inherent メソッドが backend 未対応**（「only on registered struct value」）・primitive は trait-only ゆえブロック＝enum/primitive メソッドディスパッチが入れば ext 経路は自動で乗る別件。詳細・限界は [provisional.md](provisional.md)「型システム：…拡張」。
