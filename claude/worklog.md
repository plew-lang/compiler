...
- **拡張 `#Ext`（spec/09・実装中）**：**増分1 済**（2026-06-10）＝`extension Name { impl Type { fn } }` parse＋メソッドを `Func.extId` でタグ（bare 解決から除外）＋呼び出し `value#Ext.m()`（`Kind.Hash`＋arena `Expr.ExtMethod`＋`findExtMethod`）＝同名別拡張も `#A`/`#B` で区別・test `extension_basic`/reject `extension_bare_call`。**残増分**＝(a) 型レベル `Type#Ext`（param/戻り型ビュー）(b) `#!Ext` strip・`self#!Ext`（super）(c) `defaultExtension`(d) `#P` トレイト源選択（提供メソッド衝突）(e) トレイト主語の拡張 `impl Trait`/`impl B as A`(f) 外部型コヒーレンス(g) 可視性強制(h) bare 誤呼びの front-end 診断(i) enum/primitive/generic レシーバ。詳細・限界は [provisional.md](provisional.md)「型システム：…拡張」。
