...
- **Bug15 tail-match-give が値を返さない**〔silent・既存〕：`fn f(...) -> T { match e { … => give v } }`（関数末尾の `match` を式として返す形）が `give v` を return に繋げず、戻り値が未初期化（`-Wreturn-type`）＝呼び出し側が 0/ゴミを得る。回避＝`mut val r; match e { … => { r = v } }; return r`（コンパイラ自身はこの idiom なので踏んでいない）。原因＝statement-position の `match`（`StmtAst.Match`）が tail でも式として扱われず `give` の行き先が無い。修正案＝tail の `StmtAst.Match`/`StmtAst.If` を value-position（`MatchExpr`/`IfExpr`）に lower する（または関数本体末尾式の一般化）。
- **Bug16 enum＋String payload を `match` で分解すると "where" エラー**〔loud・既存〕：`enum E { V(text: String) }` を `match` で `V(val text)` 束縛すると `type argument does not conform to the trait required by where`。String payload 特有（`Array[I32]` payload は OK）。enum 分解時の String フィールドに何かの trait 境界（Eq/Hash?）を誤って課している。`Array[I32]` payload では再現しないので Array[Enum] の回帰テストは I32 payload を使用。
