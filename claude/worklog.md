...
- **`any P` 存在型（🚧 第1増分＝正準ケース達成）**：動的ディスパッチ（vtable 相当・メンバ単位診断・異種混在）。トレイト機構の最後の大物。**設計**＝`any P` 値＝fat pointer `{ ptr data, ptr vtable }`。`data`＝rawbuf セルに boxed した具体値。`vtable`＝`(具体 struct, トレイト)` ごとの静的 thunk 配列（slot 0＝drop 予約〔現状 null＝増分1は array/String と同じ leak 厳密度で box を free しない〕・slot 1..＝各要求メソッドの thunk `(ptr self_data, args) -> ret`〔内部で具体値ロード→witness pf 呼び〕）。**thunk が要る理由**＝具体型ごとに self の by-value 表現（サイズ）が違い vtable slot signature を統一できない＝thunk が「要求シグネチャ（Self 以外は全 concrete）」へ正規化。**coercion**（concrete C→any P）＝store site（let/arg/return/配列要素）で box＋vtable wrap。**dispatch**（`s.draw()`）＝fat ptr から vtable ロード→メソッド slot index（要求順＝1+reqIdx）→fn ptr ロード→`data`＋args で call。**達成済（第1-2増分・グリーン）**：scalar `any` 束縛＋動的 dispatch・`Array[any P]` 異種混在＋`for`・`any P` 引数/戻り/**struct フィールド**・String/I64 戻りメソッド・concrete→any の暗黙 box（**6面**＝let/arg/return/field/配列要素/assign）・mut local 再代入で re-box・存在型インタフェース外メソッド呼びを loud reject（spec/08 ダウンキャスト無し）。`Array[any P]` を関数引数/戻りでも（要素 box＋境界越し要素 ref 追跡）。tests run/{any_basic,any_array,any_param_return,any_field,any_array_param_return}・reject/{any_not_in_interface,any_parameterized_trait}。**形成検査**＝`checkAnyFormation`（Driver・spec/08）＝`any P` は型引数/関連型なしのトレイトのみ許可（束縛 `any Iterator[Item=I32]`/`any Add[I32,Output=I32]` のパース＋thunk 置換が未実装ゆえ parameterized/associated トレイトは loud reject＝unbound silent dispatch を防ぐ）。**罠**＝box の型復元は `exprTypeRef`/`valueKind`/`exprLlvmType` の Method/Field 枝＋`paramLlvmTy`/`llvmFnRetTy`/`fieldLlvmTy` が `refIsAny` で anyTy を返すよう各所 hook（局所/param/フィールド/戻り/for-var の TypeRef に isAny を載せる）。**Step A 達成＝トレイト型引数/関連型付き `any`**（`any Producer[I32]`・`any Source[Item=I64]`・`any Adder[Rhs]`）＝束縛構文 `[Item=T]`/positional のパース（`TypeAst`/`TypeRef.argLabels`）＋thunk/dispatch 構築で trait typeParams/assocTypes を束縛で置換（`anyBindingEnv`→`curTypeParams`/`curTypeArgs`→`groundTypeRef`）＋vtable cache を bindKey で分離（`any Add[I32]`≠`any Add[I64]`）＋形成検査が全束縛を要求（positional/named 不足を reject）。tests run/any_parameterized・reject/{any_parameterized_trait〔未束縛 type param〕,any_unbound_assoc}。**後続増分**＝Self 返り再消去（`clone()->Self` が any 返す・現状 reject）・inout self（`any Iterator` の `inout next()`）・値意味論の box refcount/drop（現 leak）・**前提ブロック**＝Self 入力 param 準拠・Self 戻り witness は backend で pre-existing に未対応（前者＝incomplete-conformance 誤判定／後者＝compile SEGV）。
