...
✅ **プリミティブ整数の縮小 TryFrom**（spec/12）＝`<IX.convert from=wide/>`→`Result[IX, RangeError]`〔I8/I16/I32 from I64〕。`as` は全域専用ゆえ縮小は `convert` factory で range-check〔built-in `<`/`>`〕＋truncate〔新 `truncI8/I16/I32` intrinsic・`coerceInt` trunc〕。`RangeError`〔Core・export・offending 値を保持〕。test run tryfrom_narrowing。**残（trivial additive）**＝他ペア〔from I32/I16・unsigned U8/U16/U32 from U64〔truncU16/U32 intrinsic 追加〕・cross 符号〕・文字列パース TryFrom。いずれも機構は完成済で copy 増分（各 ADD→reseed→USE）。
