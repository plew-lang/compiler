...
**進行中の大物＝コンパイラ構造リファクタ**（83 フィールドの `Comp` god-object 解体＋巨大ファイル分割＋モジュール化）＝[compiler-refactor.md](compiler-refactor.md)（診断・到達点・マイルストーン M0-M4・未検証リスク）。引数名/ラベルは [spec/04 規約](../spec/01-basics/04-functions.md#引数名とラベルの指針規約) に揃える。**M0〜M3＋M5 完了**（`Comp` 11 フィールド・巨大ファイル全分割・scoped resolution）。**M4 の主目的は part 撲滅＝モジュール構造を spec/15 ベストプラクティスに沿わせる**。**方針改定 2026-06：spec/15 から旧 reason②（相互再帰は part）を撤去し「part 内は impl のみ／相互再帰は型の impl メソッド化で 1 モジュール内に収める／循環 import は常に回避可能＝設計誤り」へ**（spec/15＋design-decisions.md 同期済）。型回復統一（typed IR）は本道に不要（Backend→Frontend の型回復呼びは一方向で循環せず）＝**後段の任意トラックへ退避**。**M4-1/M4-2/M4-3 完了＝`Ir ← Frontend ← Backend ← entry` の一方向 DAG**（commit `2ddba1b`・不動点緑 run=307）：M4-1＝IR をデータモジュール `Ir.pw` へ昇格／M4-2＝逆依存3関数を Backend→Frontend/Ir へ pure-move し循環断ち Backend 独立化／M4-3＝Frontend モジュール化・root `_.pw` は `main` のみ。続いて **leaf 抽出**＝`Path.pw`（loader パス計算・`appendBytes`→Ir）／`Ops.pw`（演算子オペコード＝`opEq`/`opAdd`/`compoundBinOp`…＝`@Std/Syntax` のみ依存の純 leaf・Frontend と Backend 双方が `import ./Ops`＝op コードはどちらの層のものでもない共有語彙）／`Codegen/Array.pw`→import 化（後にメソッド化で吸収）。**理想形＝Node 風 directory+barrel**：DAG で切れる leaf/上層を import 化し、解析核を `impl Comp`／`impl LlvmCtx` へメソッド化（Comp 定義を Ir→Frontend へ移す）して part を impl 専用にする＝詳細・実装順・**メソッド化レシピ**は compiler-refactor.md「到達状態と理想形への地図」。

**作業ブランチ `refactor/methodize`（push 済・main から 3 commit）の現状**（2026-06）：(1) **Comp＋interner を Ir→Frontend へ移動**（Ir＝純データ leaf に・`impl Comp` はコヒーレンス上 Comp 定義モジュール＝Frontend にしか書けないため）＋pilot＝旧 Array 述語を `pub impl Comp` 化。(2) `Codegen/Expr.pw` を `impl Comp` 化（isDictionaryType/typeHasMethodNamed/synthDictMethod・呼出を `c.f()`・methodize した export は Backend の import 名簿から除去）。(3) **`import … with *` 廃止**（@Std/Syntax パーサで reject・spec/15＋design-decisions 同期・reject テスト追加）。各段 dev-rebuild＋test＋reseed＋不動点緑。**メソッド化の遷移則**＝呼び手が自由関数の間は methodize 先を `pub impl` に（private は呼び手も Comp メソッドのときのみ可）。残 ~630 関数＝多セッションの裾。`import` の名前列挙が 20 を超えるのはモジュール過大のサイン。

**自動化＝`tools/methodize.py`**（検証済・mixed 対応）：`python3 tools/methodize.py <file>` が c-first 関数を `pub impl Comp` へメソッド化＋呼出点を repo 全体で `RECV.NAME(…)` に書換＋export 名を報告。**mixed 対応**＝c-first 単一行シグネチャの fn だけメソッド化し非 c 自由関数・多行シグネチャは free のまま据え置き（ソース順保持）・既存 `pub impl Comp` ブロックは raw 保存。罠と手当て＝compiler-refactor.md「自動化の所見」（埋め込み型 REFUSE→module root 退避／双子は同時 methodize／`\bc\.` 語境界／export を Backend import から除去）。**✅ part=impl-only は達成＋コンパイラで強制済**（達成 `a84c9d4`／強制 `e42da9d`・不動点緑 run=307）＝Frontend（`impl Comp`・`b851108`）＋ Backend（`impl LlvmCtx`・batch 1-5＋root 集約 `a84c9d4`）の全 part が impl ブロックのみ。**強制**＝loader が「module id が既出のファイル＝part」と判定し（root は part より先にロードされる）`checkPartImplOnly` で part の TopItemAst を走査・impl/`import`/`part`/`@[...]` 以外（自由関数・型・extern・top-level val・re-export）を reject（Loader.pw）。前提として force-load される @Std を先に impl-only 化＝@Std/Syntax の AST 型＋lexer/parser 自由関数を module root（Syntax.pw）へ relocate（公開 API 不変・空 part 削除）・@Std/Core/Num の `extern` を Core root へ。テスト＝tests/part を impl-only 化＋tests/partreject/partfreefn 追加。spec/15 は「強制済」へ更新。自由関数・型・extern は各モジュール root（`Frontend.pw`／`Backend.pw`）へ集約（`tools/relocate_free.py`・`--root` 対応）。**長年の blocker「`field access only on a registered struct`」の正体＝ローカル `self` 名衝突**（Async.pw が `val self`＝promise ポインタを宣言・methodize の `st`→`self` がそれに解決）＝rename で解決・methodize_st にガード追加（詳細＝compiler-refactor.md「解決済の罠」）。op コード語彙は別 leaf module `Ops.pw`（Frontend と Backend 双方 import）。退避した自由型＝`LetEff`/`ConstInt`/`IntTy`/`ScalarCond`（Frontend.pw root）。
