...
- **Iterator 残**＝`sum`（Zero/数値タワー待ち＝大）・`enumerate`／`zip`＝**generic record feature・基盤 2-piece 完了**：
  - ✅ **(1) 型チェッカ grounding**＝`groundTypeRef` args==0 枝に **`groundRecordStruct`**〔record struct〔`(` 始まり〕を検出・各 field を ground・canonical 名再構築して登録済 concrete record の ref を返す・`recordTypeKeyFromRef`〕。
  - ✅ **(2) record-as-enum-payload 構築**＝context-less record literal は `Make` の typeStart=0〔empty 名〕で lower されるが、`buildEnumValue` が record payload field に `st.expectedTy`=grounded record を set し `genLlvmMake` が empty 名 record を `st.expectedTy` から解決。**concrete＋generic 両方 Optional/Result 越しに動く**（test run record_in_optional）。
  - **残 (3a)** chained field access on a method-result record〔`r.unwrapOr(...).value`〕＝`exprStructIdx` が method 結果 record を struct 解決せず。**残 (3b)** `EnumerateIter[I,E]` adapter＋`enumerate()` provided method＋adapter mapping＋**for-loop の record Item 分解**〔`for (val i, val v) in xs.enumerate()`〕。
