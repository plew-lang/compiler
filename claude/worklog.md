...
- **一般 Chain トレイト `?.`**（現 Optional 具体）・**`Pow`/`**`**（float 後・spec/12＝`assoc fn pow(base: Self, exp: Exp) -> Output`・invocation 形〔bare `pow(base:,exp:)` か `Type.pow` か〕は要確認）・Bool dict キー（低価値）＝各小〜中。
