...
**自動化＝`tools/methodize.py`**（検証済・mixed 対応）：`python3 tools/methodize.py <file>` が c-first 関数を `pub impl Comp` へメソッド化＋呼出点を repo 全体で `RECV.NAME(…)` に書換＋export 名を報告。**mixed 対応**＝c-first 単一行シグネチャの fn だけメソッド化し非 c 自由関数・多行シグネチャは free のまま据え置き（ソース順保持）・既存 `pub impl Comp` ブロックは raw 保存。罠と手当て＝compiler-refactor.md「自動化の所見」（埋め込み型 REFUSE→module root 退避／双子は同時 methodize／`\bc\.` 語境界／export を Backend import から除去）。**Frontend 側 methodize 完了**＝Codegen/Parser 配下の c-first 自由関数はほぼ全て `impl Comp` メソッド化済（branch `refactor/methodize`・main から多数 commit・各段不動点緑 run=307）。退避した自由型＝`LetEff`/`ConstInt`/`IntTy`/`ScalarCond`（Frontend.pw root）。**残トラック2つ**：(A) **Backend/Llvm（32 ファイル）を `impl LlvmCtx`**（受け手 `st: inout LlvmCtx`・Comp は引数のまま＝script を st 版に要改造）。(B) **part 内に残る非 c 純ヘルパ ~66 個を module root か別モジュールへ**（part=impl-only の最終形。最大は `Codegen/Ops.pw` の op コード 32 個＝Backend も import 中＝leaf module 化 or Frontend root 退避の判断要）。
