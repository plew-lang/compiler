...
**自動化＝`tools/methodize.py`**（検証済・バグ修正済）：all-c ファイル（全 top-level fn が `c: inout Comp` 先頭）を `python3 tools/methodize.py <file>` で一括メソッド化＋呼出点を repo 全体で `RECV.NAME(…)` に書換＋export 名を報告。**ガード**＝mixed ファイル（allc≠total）は refuse（非 c 関数の取りこぼし防止＝手当て要）。**twin（同名が複数定義＝オーバーロード）ファイルは不可**（blanket 書換が双方の呼出を巻き込む＝事前に `grep -rlE "^(export )?fn NAME\("` で確認）。バッチ手順＝twin/mixed でない all-c を複数 run→export 名を Backend の `import ./Frontend` から除去→dev-rebuild/test/reseed/不動点→commit。**メソッド化済（〜24 ファイル・〜166 関数・branch 12 commit）**：Codegen の大半＝Expr/Operator/Mono{Index,Collect,Resolve,Scan,FreeFn}/Check{Match,Conformance,Return,Duplicate,Generic,Call}/Verify{Stmt,Call,Ops,Func,Decl}/Infer{Type,Check}/Lower{Impl,Decl,TopLevel}/Resolve{Operator,MatchBind}/Gen。**残り Codegen**（safe な all-c だが大きい）＝`Mono/Ground`(15)・`Check/Bounds`(18)・`Resolve/Locals`(22)・`Codegen/Decl`(23)・`Resolve/Module`(24)・巨大 `Resolve/ExprType`(exprType)・`Lower/Expr`(lowerExpr/lowerExprWithCtx)。**mixed/twin で手当て要**＝`Check/Make`(makeFieldType twin)・`Lower/Params`(mixed)・`Parser/Decl`・`Codegen/Emit`・`Codegen/Ops`・`Codegen/Stmt`・`Lower/Types`・`Lower/Match` 等（非 c 関数や twin を含む＝script 不可・手当て or script 拡張）。**Backend/Llvm（32 ファイル）は別＝`impl LlvmCtx`**（受け手が `st: inout LlvmCtx`・script を st 版に要改造／c 専用関数は Comp メソッド）。Backend の `import ./Frontend` 名簿は methodize で縮小中。
