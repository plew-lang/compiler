...
- 🔄 **step 5b＝提供メソッド `.map()`/`.filter()` の sugar**（`xs.map(f:…)`／lazy・[[lazy-iterators-direction]]）。**`Self`/`Item` 型解決の土台＝実装済**（commit 群）：`Comp.curSelfRef/curItemRef`＋`resolveTy` が裸 `Self`→準拠型・`Item`→その `type Item` 束縛に解決（`Item` は curItemRef!=0 の Iterator メソッド env 下のみ＝ユーザー型名 `Item` を壊さない）。`setSelfItemEnv`/`clearSelfItemEnv` を codegen(genFunc)・scan(collectGenInsts/scanGenInstFields)・proto loop(_.pw)・emitMonoMethod proto に配線。`Func.isProvided`＝instantiateProvidedMethods のコピーに付与（scanGenInstFields の method scan は skip＝自己 cascade 防止）。`tyRefIsGround` は `Self`（文脈キーワード）を非 ground 扱い＝flat Make/Cast scan が提供 body の `<FilterIter[Self,…]>` を誤登録しない。**現状＝`filter` を具体準拠型（Counter）に載せる経路は解決まで通る**。
  - **残る本丸＝提供 self-returning メソッドの demand-driven 単相化**。`filter` は `impl Iterator` 経由で**全準拠型にコピー**され、generic 準拠型 FilterIter にも載る→`filter` の戻り `FilterIter[Self,…]` が `FilterIter[FilterIter[…]]` に展開され**呼ばれずとも eager 発行で無限カスケード**。正攻法＝**呼び出し時のみ単相化**（戻り adapter inst が登録済のときだけ発行＝reachability skip）。`isRegisteredGenInst`/`providedRetReachable`＋emitMonoMethods/_.pw に skip を入れる試作はしたが check-time（checkMethodBounds line 21 bound）と相互作用するバグで撤回＝**foundation commit（9a034d2）に戻して green 維持**。次：demand-driven skip を check と矛盾しない順序で入れる（checkMethodBounds も到達不能な提供 inst を検査しないよう揃える）。
  - **`map[B]` は更に method-level 型パラメータ**（出力型 B）が要る＝`scanExprInsts` の Method はメソッド instantiation を登録しない＝generic method on concrete receiver 未対応。`filter` は B 不要（出力 Item）なので**先に filter を通すのが順当**。
