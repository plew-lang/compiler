...
- ✅ **step 5f＝任意の順の連鎖 完了**（`xs.filter(p).map(f).filter(q)`・`map().map()`・任意順）。**generic メソッド × generic レシーバ**（`map[B]` を `FilterIter[…]` 上で）を **(receiver inst, method-own 型引数) ごとに単相化**：`FnInst.recvInstRef`／`registerMethodInst` が generic レシーバ＋method-own 有りを per-call 登録（full args＝receiver args ＋ 推論 own args）／`emitMonoFn` が `curRecvInstRef` セット／mangle suffix は method-own tail のみ（`writeMethodInstSuffixFrom`・receiver mangle が残りを担当・def=genSignature／call=Expr.Method 一致）／`exprType(Method)` も generic レシーバで method-own を束縛。提供メソッドの generic 準拠型継承（`providedCopyTypeParams`）＋連鎖結果型の scan 登録（`scanExprInsts`）も。test `iter_chain`/`iter_chain3`/`iter_chain_mapmap`。
- 🔲 **step 5g＝拡充**：`reduce`/`fold`（終端・eager）・`take`/`enumerate`/`zip`（同型 additive）・`String`/`Format` 出力の map（`String: Format` 要）・複数行メソッドチェーン構文（改行前 `.` パース）・Array/Range を Iterable に。
