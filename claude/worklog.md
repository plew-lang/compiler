...
**自動化＝`tools/methodize.py`**（検証済・mixed 対応）：`python3 tools/methodize.py <file>` が c-first 関数を `pub impl Comp` へメソッド化＋呼出点を repo 全体で `RECV.NAME(…)` に書換＋export 名を報告。**mixed 対応**＝c-first 単一行シグネチャの fn だけメソッド化し非 c 自由関数・多行シグネチャは free のまま据え置き（ソース順保持）・既存 `pub impl Comp` ブロックは raw 保存。罠と手当て＝compiler-refactor.md「自動化の所見」（埋め込み型 REFUSE→module root 退避／双子は同時 methodize／`\bc\.` 語境界／export を Backend import から除去）。**Frontend の part は全て impl-only 達成**（commit `b851108`・不動点緑 run=307）＝c-first 自由関数を全て `impl Comp` メソッド化（＋Loader の `parseAndLowerFile`）＋残る非 c 純ヘルパ 29 個を `tools/relocate_free.py` で Frontend.pw root へ verbatim 退避（同一モジュール scope＝call site 不変）＋`runFrontend` を root へ移し空の `Codegen/Driver.pw` 削除。op コード語彙は別 leaf module `Ops.pw` に確定済（Frontend と Backend が双方 import）。退避した自由型＝`LetEff`/`ConstInt`/`IntTy`/`ScalarCond`（Frontend.pw root）。**残トラック＝(A) Backend/Llvm（32 ファイル・303 自由関数）を `impl LlvmCtx`**（受け手 `st: inout LlvmCtx`・Comp は引数のまま＝`tools/methodize_st.py`）＝**backend codegen の穴で保留中**（self フィールドアクセスが一部フレームで落ちる・根本未特定＝compiler-refactor.md「Backend track」）。
