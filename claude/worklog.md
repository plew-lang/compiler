...
- **グリーン**：`./test.sh`（run261/panic13/reject268/part5/fail=0＋不動点）／`./test-gen.sh`（gen10/reject1）。
...
- **一般 Chain トレイト `?.`**（現 Optional 具体）・**`Pow`/`**`**（float 後）・**`Output != Self`**・**`guard` 文**（条件チェーン束縛・parser 未）・**match ガード/ネストパターン**・**global 前方参照**・Bool dict キー（低価値）＝各小〜中。
