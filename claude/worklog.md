...
- **M3 パッケージ管理**：仕様確定＝[spec/17-packages](../spec/04-execution/17-packages.md)＋design-decisions「パッケージ」（manifest 3 項目／`/` は名前の字面・公開面はフラット（ルート `export`）／依存表記（members を `/` パスで選択・`as` は member 内のみ・`commit`）／桁数バージョン／最新互換・複数版共存・phantom 禁止／lock・ワークスペース＝複数メンバ単一 repo・1 バージョン／ソース配布／ネイティブ依存＝`[[native]]` 配列・C/C++/Rust/pkg-config 祝福・C/C++ は `sources`/`include`/`defines`/`std` 構造化（生フラグ＝ACE ゆえ不可）・ビルドスクリプトなし）。実装は未着手（導入後に `@Std/Syntax` を in-tree→外部共有パッケージへ昇格）。
