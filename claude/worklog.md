...
> - **F4〔中・✅struct 分解は実装済〕** struct 分解パターン `P { val x, val y }`（spec/11:70,129 正典）。match は実装済（`run/struct_pattern`）。**for ヘッダの struct 分解も実装**＝`Stmt.For` に `isStruct/patStart/patLen/binds` を追加・`parseFor` が `Ident {` を `parsePattern` で取り各反復で要素フィールドを束縛（punning/rename/discard 可・パターン型不一致と range 反復は loud reject・heap フィールドは borrow＝ASan clean）。test `run/for_struct_destructure`・`reject/for_destructure_mismatch`。**残（未実装機能・非バグ）**＝for の tuple 分解（dict 反復子/record 要素要）・`guard` 文（条件チェーン束縛ごと別 feature）。→ review-items。
