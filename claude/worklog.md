...
- **一般 Chain トレイト `?.`**（現 Optional 具体・実装者は Optional のみ＝spec も Result 除外ゆえ実用価値低）・**`Pow`/`**`**（float 後・spec/12＝`assoc fn pow(base: Self, exp: Exp) -> Output`・invocation 形〔bare `pow(base:,exp:)` か `Type.pow` か〕は要確認）＝各小〜中。
  ...
  - **残＝ユーザー struct を dict キーに**（`Dictionary[Pair, I64]` with `@[Hash]` derive）＝**別の大物**：derive は 2-phase の `plew gen` ワークフロー依存で、app が dict を使うと gen harness compile 時に `Pair: Hash` 未生成で conformance fail（chicken-and-egg）。Bool dict キーとは別系統（gen 2-phase ＋ conformance）＝要 focused。
