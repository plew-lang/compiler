...
- **`From`/`TryFrom`**＝ユーザー型は✅完了：`factory(from:S)`＋`x as T`/`<T from=x/>`（From）・`result[E] factory convert(from:)`＋`<T.convert from=…/>`（TryFrom）・**`try` の異エラー型 From 変換**〔E≠E' のとき E の `from` factory で E'→E 変換・`typeRefHasFromFactory`/`typeRefFromFactoryIdx`/`resultErrArgRef`・genLlvmTry Err 枝で from factory 呼び・Verify は from factory ある時のみ E≠E' 許可〕＝✅。〔✅ mixed float context・✅ float→int `as` の clean reject〕。**残＝プリミティブ数値の縮小/パース TryFrom**（intrinsic-backed＝`I64→I32` 等の可謬変換・`U8.convert(from:)`）＝**focused session（複数 sub-gap）**。試行で判明した walls：(1) builtin に bare inherent impl 不可〔`pub impl I8 { factory }`→「cannot define methods on a built-in」`Check.pw:2852`〕＝trait conformance 必須〔`pub impl I8 as TryFrom[I64]` 形〕。(2) **factory-as-trait-requirement 未配線**＝`trait TryFrom { result[Error] factory convert(from:) }` の factory 要求が `tr.reqs` に witness 登録されず→`methodWitnessesAConformance(I8,convert)` false→同 builtin check で reject。**要るもの**＝trait の factory 要求を reqs に登録＋conformance witness 照合を factory 対応・truncI8/I16/I32 等の narrowing intrinsic〔`coerceInt` で trunc・`truncU8` と同型〕・`RangeError` 型〔Core・ambient 一覧 TODO と関連〕・range-check は built-in `<`/`>` で OK（intrinsic 不要）。
...
- **一般 Chain トレイト `?.`**（現 Optional 具体・実装者は Optional のみ＝spec も Result 除外ゆえ実用価値低）・**`Pow`/`**`**（float 後・spec/12＝`assoc fn pow(base: Self, exp: Exp) -> Output`・invocation 形〔bare `pow(base:,exp:)` か `Type.pow` か〕は要確認）＝各小〜中。
