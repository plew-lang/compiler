...
- **グリーン**：`./test.sh`（run204/panic11/reject109/part5/fail=0＋不動点）／`./test-gen.sh`（gen10/reject1）。
...
- **Iterator 拡充**：終端 `count`/`fold`/`any`/`all` ＝✅ **実装済**（`pub impl Iterator` の provided method・`mut val it = self`〔`: Self` 注釈を付けると型追跡が切れて dispatch 不可ゆえ注釈なしで `self` の型から復元〕で `inout next()` を駆動・**直接の具体イテレータで動作**・test run/iter_terminals）。**残ギャップ（重要・別機構）**＝**終端を lazy adapter（`MapIter`/`FilterIter`）受信で呼ぶ**（`xs.map(f).fold(...)`）と「provided method を generic-instance 受信で単相化」が未対応で loud reject（`for v in xs.map(f).filter(p)` の lazy チェーンと `.map().filter()` 構築は動くが、**終端を adapter に繋ぐと** `ensureProvidedDeclared`/`ensureGenMethodDeclared` が generic-instance Self の provided method を declare できず落ちる＝mono 機構の拡張が要る）。`forEach` は `fn(x)->()` の unit 返り fn 型がパーサ未対応ゆえ保留（`for` で代替可）。未実装＝`reduce`/`sum`/`collect`/`first`＋遅延 adapter（take/zip/enumerate/skip）。
