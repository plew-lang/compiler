...
- **グリーン**：`./test.sh`（run219/panic12/reject124/part5/fail=0＋不動点）／`./test-gen.sh`（gen10/reject1）。
...
- **Iterator 拡充**：終端 `count`/`fold`/`any`/`all` ＝✅ **実装済**（`pub impl Iterator` の provided method・`mut val it = self`〔`: Self` 注釈を付けると型追跡が切れて dispatch 不可ゆえ注釈なしで `self` の型から復元〕で `inout next()` を駆動・**直接の具体イテレータで動作**・test run/iter_terminals）。**残ギャップ（重要・別機構＝nested provided-method emission）**＝**終端を lazy adapter（`MapIter`/`FilterIter`）受信で呼ぶ**（`xs.map(f).fold(...)`）が loud reject（「this method is not emittable yet (generic / provided / trait receiver)」）。**精密診断**：(1) **`exprResultRef` の Method 対応は済**＝`val it = xs.map(f)`／`val o = d.get(k)`（無注釈）が generic instance 戻りを追跡（test run/method_result_bind）。(2) bound adapter への `it.next()`（trait 要求）は**動く**（test 確認・MapIter.next() は単相化される）。(3) **落ちるのは provided 終端 `count`/`fold` を MapIter 受信で**＝`count` は `instantiateProvidedMethods` で MapIter にコピーされ typeParams=[I,F,B]・`findMethod(MapIter,count)` も当たるはずだが、emission が通らない（推定＝count[Self=MapIter] 本体内の `mut val it = self; it.next()` のネスト単相化、または provided-copy の `declareOneGenMethod` 経路）。`ensureProvidedDeclared`（非 generic 用）と `ensureGenMethodDeclared`（generic instance 用）の provided-method×generic-instance-Self 経路の配線が要る。要・実機 print デバッグ。`forEach` は `fn(x)->()` の unit 返り fn 型がパーサ未対応ゆえ保留（`for` で代替可）。未実装＝`reduce`/`sum`/`collect`/`first`＋遅延 adapter（take/zip/enumerate/skip）。**境界の精密再キャリブレーション（2026-06-11）**：✅ **bare ユーザー Iterator は全面動作**＝`for val x in counter`（test it3）／`counter.count()`／`counter.fold(init:,combine:)`（test it5・5/10 を確認）。✅ **adapter の for 反復も動作**＝`for val x in counter.map(f)`（user Iterator の `map` 戻り MapIter を for-iterand に・60 を確認）。**⚠ 注意：Array は std で Iterable/Iterator 非準拠**＝`xs.iterator()`/`xs.map()` は raw 配列に無い（adapter は `Iterator` トレイトの provided メソッドゆえ user Iterator 上のみ・配列直接は index-walk for）。✗ **残る唯一のギャップ＝adapter 受信の provided 終端**：`counter.map(f).fold(...)`／`.count()`。**2 層の別原因**：(1) **`fold[Acc]`＝メソッド固有型パラメータ**ゆえ `ensureGenMethodDeclared` の arity 検査（`mf.typeParams.count == instRef.args.count`）が落ちる（MapIter[I,F,B]=3 args vs fold copy の typeParams≠3）＝**method-own type param を call 引数〔`init: Acc`〕から推論し instance env〔I,F,B〕と合成して declare/emit する FnInst 流機構が要る**。(2) **`count()`＝own param 無し**ゆえ declare は通るが**本体 `mut val it = self; it.next()` の nested emission が落ちる**（self=MapIter の `it.next()`〔MapIter.next・trait 要求〕が nested 文脈で `ensureGenMethodDeclared(next, MapIter)` に未ルート＝「this method is not emittable yet (generic/provided/trait receiver)」Llvm.pw:448 系）。**核心**＝(A) provided メソッド本体内の generic-instance レシーバ method 呼び（nested `it.next()`）を ensureGenMethodDeclared 経由に、(B) method-own type param（`fold[Acc]`/`map[B]`）を generic-instance 受信で call-site 推論して declare/emit。両者で adapter 終端チェーンが解禁（要・focused セッション・大物）。
