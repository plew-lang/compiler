...
- **newtype 非 int underlying のメソッド/フィールド継承**（`userId.bytes`〔UserId=String〕）＝中・risk（`typeOf` が型チェッカ distinction と codegen 解決の両用ゆえ blanket 不可・codegen 専用解決点の個別配線・過去に field access で revert）＋ unique/deinit/factory 継承・`export newtype`。int underlying は完備。**精密な gap map（試行で判明）**：(1) `as` re-tag〔`"a" as UserId`・unwrap `u as String`〕は backend `Expr.Cast` が「int-width only」で reject＝non-int は `newtypeUnderRef != 0` なら値そのまま返す no-op で良い〔construction は試作で動いたが unwrap は (2) に依存〕。(2) **newtype local の型追跡**＝`val u: UserId = …` の locTypeRef が UserId を保たず exprTypeRef(u) が underlying/0 を返す疑い→unwrap `u as String` の operand-is-newtype 判定が効かない。(3) **メンバ継承**＝`u.bytes`〔String.bytes〕は `isStringBytesField` が newtype-over-String を受けず・一般メソッドも newtype→underlying 解決点を field access/method dispatch 各所に配線要。(1) は単独で clean だが (2)(3) なしでは construct-only で無用ゆえ partial revert 済。**要 focused session**（type-tracking ＋ 継承の多点配線をまとめて）。
- **Iterator 残**＝`sum`（Zero/数値タワー待ち＝大）・`enumerate`／`zip`＝**generic record の deep multi-piece feature**。試行で**精密 3-piece map** 判明：
  - **(1) 型チェッカ grounding〔解法確定〕**＝generic record `(index:U64,value:E)` が canonical struct（args 無し・名前に型を焼込）ゆえ `groundTypeRef` が field E を ground しない→`Optional[(index:U64,value:E)]` の use が型不一致。**修正＝`groundTypeRef` の args==0 枝に `groundRecordStruct`**〔record struct〔名前 `(` 始まり〕を検出・各 field を ground・canonical 名を再構築して登録済 concrete record の ref を返す〕＋backend `genLlvmMake` の `structRegSlot` を grounded 名で引く。試作で型チェック通過・全 suite green〔safe〕だが (2)(3) 未完ゆえ revert 済。
  - **(2) record-as-enum-payload 構築〔backend gap・concrete でも未対応〕**＝`<Optional.Some v=(index:5,value:7)/>` が SIGSEGV でなく構築 reject。**根因＝context-less record literal は `Make` の typeStart/typeLen=0〔empty 名〕で lower〔`Lower.pw:752`〕**＝enum payload field value に variant payload 型が context 伝播されず。**修正案**＝(a) lower 側で variant payload 型を field value の context に伝播〔`lowerMakeFields`/`lowerExprWithCtx`〕 or (b) backend `genLlvmMake` が empty 名 record で `st.expectedTy`〔genLlvmMakeEnum が payload 型に set〕or field 値型から canonical 名を解決。
  - **(3) for-loop の record Item 分解**〔`for (val i, val v) in xs.enumerate()`〕＝未検証。
  - `zip`（第2要素型＝`J::Item`＝generic 抽象 `T::Item` 未対応ゆえ表現不能・別系統）。
