...
- **グリーン**：`./test.sh`（run270/panic13/reject271/part6/fail=0＋不動点）／`./test-gen.sh`（gen10/reject1）。
...
- **一般 Chain トレイト `?.`**（現 Optional 具体）・**`Pow`/`**`**（float 後）・**`guard` 文**（条件チェーン束縛・parser 未＝match ガードとは別物）・**enum-dispatch の match capture／ネストパターン**・Bool dict キー（低価値）＝各小〜中。
