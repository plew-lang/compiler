...
- **一般 Chain トレイト `?.`**（現 Optional 具体・実装者は Optional のみ＝spec も Result 除外ゆえ実用価値低・**factory-as-trait-requirement キーストーン済ゆえ unblock**＝`trait Chain { type Value  fn chain()  factory fromValue/empty }` が書けるように）・**`Pow`/`**`**（float 後・spec/12＝`assoc fn pow(base: Self, exp: Exp) -> Output`・invocation 形〔bare `pow(base:,exp:)` か `Type.pow` か〕は要確認）＝各小〜中。
  ...
  - **残＝ユーザー struct を dict キーに**（`Dictionary[Pair, I64]` with `@[Hash]` derive）＝**別の大物**：derive の 2-phase `plew gen` で、phase 1（harness compile）が dead user main の `Dictionary[Pair]` 使用を処理して `Pair: Hash` 未生成のまま fail（chicken-and-egg）。**精密な根因**＝gen mode で dead user main は emission では skip されるが（`Backend/Llvm.pw` `c.genMode` 分岐）、front-end は skip しない：(1) `checkFnBounds`/`checkMethodBounds`〔conformance bound〕(2) `collectGenInsts` が **`c.stmts` を GLOBAL に walk**〔`Mono.pw:1072` の Let 型注釈スキャン〕して dead main の `val d: Dictionary[Pair,_]` から genInst 登録→methods emit→`Pair.hash` not found。**clean fix**＝gen mode で arena stmt/expr を func 帰属させ dead user main を全 front-end pass で除外する refactor（func 単位 skip だけでは global c.stmts walk が漏らす）。試行は revert 済。要 focused session。
