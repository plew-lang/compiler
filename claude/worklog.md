...
4. 🔄 **ARC ＋ `Ref`/`WeakRef`**：✅ 基本 `Ref[T]`。✅ **スコープフレーム基盤**＝`genBlock`/`for`/`match` 入口で `scopeMark`→出口で `scopeExit`（`emitScopeDrops`〔今は no-op〕＋`popLocals`）。名前解決を真の lexical scope に厳格化（回帰テスト `scope_shadow`）。✅ **ASan ハーネス**（`ASAN=1 ./test.sh`・free 導入時の UAF/double-free を機械検証・leak 検出は free 後に `detect_leaks=1`）。
   ...
   - **CoW（struct）残り**：`Struct_share`（heap フィールド retain）を足し、struct 束縛（`genCopyValue` の struct 枝＋`Stmt.Let` immutable struct の `genExpr`）を share に切替・mono struct/enum も。これで struct も共有になり、**owned struct ローカル release が安全に再有効化**（前回 double-free でクラッシュした件が解消）。
   - その後に安全化：**再帰 release**（配列の struct 要素・現状 `_copy` 浅いので copy も深くする必要）・**早期 exit release**（`return`〔値を temp 退避〕/`break`/`continue`・`panic` は drop 不走）・**`Ref` 箱の refcount 化**（Expr.pw の malloc）。
