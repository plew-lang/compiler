...
> **次の一歩＝(2) closure キャプチャ**（イベントループ第1段・Iterator コンビネータの前提）。✅ **traits step 1 完了**＝T1（trait 宣言＋`impl Type as Trait` body/`via` witness＋完全性チェック）・T2（提供メソッド）・T3a（generic free 関数の呼び出し位置単相化）・T3b（`where T: Trait` 境界＋強制）・**assoc fn**（`assoc fn`＝静的メソッド・`Type.name(args)`）・**Eq/Ord 配線**（`== != < <= > >=` をユーザー型の `eq`/`compare` witness へ脱糖・Ord は Ordering タグ検査）。タグ `traits-t1`〜`traits-t3`/`assoc-fn`/`traits-eq-ord`。**既知ギャップ（additive）**：`via` 先実体の存在/シグ未検証・未知トレイト名準拠は無視・完全性は名前一致のみ・Eq/Ord は assoc fn 名で検出〔`as Eq` 準拠ゲートなし＝寛容〕・enum の Eq/Ord witness は未（enum は tag 比較のまま）・generic free 関数の transitive 単相化／明示型引数／generic メソッドの `where` 強制は未・generic テンプレート本体は未インスタンス時に型検査しない（既存 generics と同方針）。詳細は item 7・spec/08,12。
