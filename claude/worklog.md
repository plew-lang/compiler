...
- **`any P` 存在型（spec/08）＝完成**（「完了した大物」参照・残ギャップは下記の任意のみ）。**残り（backend 全体水準・任意）**＝concrete→any boxing の deep-copy（現状 shallow＝backend 全体の by-value 非コピーと同水準・simple/String フィールドは観測同一）・box の drop（rc/free＝backend 全体の array/String leak と同水準）。**前提機能ブロックを 1 つ解消**＝平易メソッドの `-> Self` 未解決 SEGV を Step B で修正済（Self 環境を method emission で設定）。
