...
- ~~**条件チェーン＋`guard` 文**~~＝✅**ほぼ完了**（spec/11・match ガード revert の正規代替）。**if-let**〔`if PATTERN = expr [&& boolClause…] { then } [else]`〕・**while-let**〔`while PATTERN = expr { body }`〕・**`guard` 文**〔`guard cond/PAT=e { else } rest`＝rest を成功枝に包んで binding-escape〕＝全部済（**パーサで `match`/`if`/`while` へ脱糖・arena/backend 変更ゼロ**・`detectRefutableBinding` backtrack・scrut は `parseBinAst(minPrec:3)` で top-level `&&` 手前まで・trailing `&&` Bool 節は parens で `||` 内包〔spec 通り〕）。test run if_let/while_let/guard_stmt。~~Bool-first-then-binding〔`c && P=e`〕~~＝✅（`condChainHasBinding` で `&&` 全クローズを走査・binding がどのクローズにあっても cond-chain 脱糖へ・pure-Bool は plain-expr 経路維持で `||` 対応・if/if-expr/while/guard 4 箇所・test run cond_chain_bool_first）。残〔小〕＝(c) **guard else の発散 enforce**〔現状は包む脱糖ゆえ非発散でも正しく動くが spec は発散必須〕。
- **一般 Chain トレイト `?.`**（現 Optional 具体）・**`Pow`/`**`**（float 後・spec/12＝`assoc fn pow(base: Self, exp: Exp) -> Output`・invocation 形〔bare `pow(base:,exp:)` か `Type.pow` か〕は要確認）・**match ネストパターン**（`Some(Point{x,y})`＝再帰 destructure・PatternAst.binds を nested pattern 化＋脱糖）・Bool dict キー（低価値）＝各小〜中。
