...
- **🔴 generic/trait-template 本体の abstract 検査＝Rust 流で実装すると決定（2026-06-12・最大の残 soundness 作業）。設計＝[generic-abstract-checker.md](generic-abstract-checker.md)・spec/06「ジェネリック本体の型検査」**。型パラメータ `T` を不透明型として扱い `where` bound が提供する capability だけ許す（per-instance ではなく宣言時・C++ template 流は不採用）。**土台は #9「eager generic type-checker」として部分実装済み**（`checkGenericBodies`/`checkBoundsExpr`・`isTypeParamOf`〔関数スコープ〕・`boundProvidesMethod`/`traitProvidesMember`〔supertrait 込み〕）＝既にメソッド呼び・比較は bound 要求。**進捗＝✅P1〔演算子の bound 要求＝Add/Sub/.../Neg/Not/BitNot/Index〕・✅P2〔型パラメータ strict 同一性・isConcreteTypeName で「T vs concrete=reject／T vs 他 generic param=unify lenient」〕・✅P3〔全位置＝return/let/call/make/代入/配列要素/method引数/if-match-give＋concrete binop〕・✅P4〔field/index access on T〕。型パラメータ周りの Rust 流検査は実用上完成。残＝(a)🔴 trait provided-method 本体の未検査〔Self を abstract 扱いで本体検査＝次の本命〕・(b)`val y: T = 5` の literal-context・(c)関連型 `T::Item`〔additive〕**。詳細＝[generic-abstract-checker.md](generic-abstract-checker.md)。concrete な型エラー（`fn f[T](x: T)->I64 { val y: I64 = true …}`／`fn f[T](...)->I64 { return "s" }`／provided method 本体）が全て素通し＝**async と同類だが型パラメータがあるため難しい**。`tcCheckValue` は `knownNamedTypeId`→0 で T を lenient に通すが、**他の verify 検査（category/`checkLitSpan`）は型パラメータ非対応**ゆえ単純に template を verify すると `T` 値が concrete 位置へ流れる正当な generic コードを誤 reject（コンパイラ自身が generic 多用＝self-host 破壊リスク大）。**正しい直し方＝(a) 単相化インスタンスを substitution env 付きで verify**＝concrete 型で検査ゆえ false-reject も weakening もない＝これが本筋。**(b) template を型パラメータ-aware〔T を abstract 扱い〕で verify は✗**＝2026-06-12 診断で実証：`checkLitTi` に type-param skip を足し generic-function verify を有効化→**5 regression〔3 false-reject〔run cow_struct_array_elem/derive_eq_struct/generic_bound_primitive〕＋2 weakening〔reject closure_arity_mismatch/print_bare_literal が accept 化＝type-param skip が広すぎ既存検査を緩めた〕〕**＝template+leniency は false-reject と soundness 緩めをトレードするだけで不可・revert 済。async（concrete subset）は ✅。**よって per-instance verify〔mono が見つける各インスタンスを substitution env で walk〕が要・emission と verify の分離を跨ぐ設計が要る**。
...
> 受理健全性は広く確認済（int/float/幅/sign/struct/Optional/enum payload/Ref・引数〔free/method/assoc/fn-value〕/代入/複合代入/比較/論理/return/配列要素〔ネスト・非リテラル値含む〕/dict literal+全 K/V メソッド/append 要素/range 両端/field & default arg/関数値署名〔全位置〕/generic 型 arity/準拠 witness 返り値/if-while 条件 Bool/void 値使用/`as` の Bool↔numeric/単項 String/call-site inout/move/unique/可視性/overflow/missing-return/網羅性/async 本体/try error 型をすべて reject・~80 probe で contained 穴は概ね尽きた）。**残る既知の soundness 穴**＝①🔴 **generic/trait-template 本体未検証**〔上記 A・最大・要 per-instance 検査 or 型パラメータ-aware 化〕②**use-after-move**〔flow 解析要〕③**field default の他フィールド/self 参照**〔backend で構築時 catch・未構築なら無検出・低 harm〕④generic-receiver method の **T-param** value 引数 grounding〔横断 refactor 連動・concrete primitive は✅〕。
