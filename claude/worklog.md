...
- **グリーン**：`./test.sh`（run178/panic11/reject103/part4/fail=0＋不動点）／`./test-gen.sh`（gen10/reject1）。
...
- **`any P` 存在型（🚧 第1増分＝正準ケース達成）**：動的ディスパッチ（vtable 相当・メンバ単位診断・異種混在）。トレイト機構の最後の大物。**設計**＝`any P` 値＝fat pointer `{ ptr data, ptr vtable }`。`data`＝rawbuf セルに boxed した具体値。`vtable`＝`(具体 struct, トレイト)` ごとの静的 thunk 配列（slot 0＝drop 予約〔現状 null＝増分1は array/String と同じ leak 厳密度で box を free しない〕・slot 1..＝各要求メソッドの thunk `(ptr self_data, args) -> ret`〔内部で具体値ロード→witness pf 呼び〕）。**thunk が要る理由**＝具体型ごとに self の by-value 表現（サイズ）が違い vtable slot signature を統一できない＝thunk が「要求シグネチャ（Self 以外は全 concrete）」へ正規化。**coercion**（concrete C→any P）＝store site（let/arg/return/配列要素）で box＋vtable wrap。**dispatch**（`s.draw()`）＝fat ptr から vtable ロード→メソッド slot index（要求順＝1+reqIdx）→fn ptr ロード→`data`＋args で call。**達成済（第1-2増分・グリーン）**：scalar `any` 束縛＋動的 dispatch・`Array[any P]` 異種混在＋`for`・`any P` 引数/戻り/**struct フィールド**・String/I64 戻りメソッド・concrete→any の暗黙 box（**6面**＝let/arg/return/field/配列要素/assign）・mut local 再代入で re-box・存在型インタフェース外メソッド呼びを loud reject（spec/08 ダウンキャスト無し）。`Array[any P]` を関数引数/戻りでも（要素 box＋境界越し要素 ref 追跡）。tests run/{any_basic,any_array,any_param_return,any_field,any_array_param_return}・reject/{any_not_in_interface,any_parameterized_trait}。**形成検査**＝`checkAnyFormation`（Driver・spec/08）＝`any P` は型引数/関連型なしのトレイトのみ許可（束縛 `any Iterator[Item=I32]`/`any Add[I32,Output=I32]` のパース＋thunk 置換が未実装ゆえ parameterized/associated トレイトは loud reject＝unbound silent dispatch を防ぐ）。**罠**＝box の型復元は `exprTypeRef`/`valueKind`/`exprLlvmType` の Method/Field 枝＋`paramLlvmTy`/`llvmFnRetTy`/`fieldLlvmTy` が `refIsAny` で anyTy を返すよう各所 hook（局所/param/フィールド/戻り/for-var の TypeRef に isAny を載せる）。**Step A 達成＝トレイト型引数/関連型付き `any`**（`any Producer[I32]`・`any Source[Item=I64]`・`any Adder[Rhs]`）＝束縛構文 `[Item=T]`/positional のパース（`TypeAst`/`TypeRef.argLabels`）＋thunk/dispatch 構築で trait typeParams/assocTypes を束縛で置換（`anyBindingEnv`→`curTypeParams`/`curTypeArgs`→`groundTypeRef`）＋vtable cache を bindKey で分離（`any Add[I32]`≠`any Add[I64]`）＋形成検査が全束縛を要求（positional/named 不足を reject）。tests run/any_parameterized・reject/{any_parameterized_trait〔未束縛 type param〕,any_unbound_assoc}。**Step C 達成＝inout self メソッドの any ディスパッチ**（`inout fn bump()`・`any Iter[Item=I64]` の `inout next()->Optional[Item]`）＝要求 Func に `selfInout` を保持（`SigAst.selfInout`・`parseSigAst(selfInout)`・`inout fn` 要求で true・`lowerSigToFunc`）＋thunk が inout 要求では self を box の ptr で渡す（in-place 変異が box 共有で永続）＋`exprTypeRef` の Method 枝が any-receiver の grounded 戻り ref を返す（`match it.next()` が `Optional[I64]` を復元）。tests run/{any_inout,any_iterator}。**罠**＝arena Expr.Method の match で `args:` フィールドは punning ゆえ rename は `args: val margs`（`val margs` は非存在フィールド束縛で self-compile を壊す＝WIP バイナリは緩く通し種コンパイラが loud に弾く）。**Step B 達成＝Self 返り再消去**（`fn dup()->Self` が `any P` を返す・spec/08）＝まず前提の **pre-existing「平易メソッドの `-> Self` 未解決 SEGV」を修正**（`genLlvmFuncBody`/`declareUserFns` でメソッドに `setSelfItemEnv(recvInstRef=0)`＝Self→受信型・`llvmFnRetTy`/`paramLlvmTy` に `curSelfRef` 時の `Self`→`llvmTypeOfRef(curSelfRef)` 分岐）→ thunk が `-> Self` 要求で witness の concrete 結果を同 existential に re-box（戻り型＝anyTy・`anyMethodRetRef` が anyRef を返す）。Self 入力 param のみ非呼出（`reqUsesSelfInInput`）。vtable は global を**先に生成→cache→thunk 構築**で re-erasure の自己再帰回避。tests run/{self_return,any_self_return}。**Step D 達成＝値意味論（コピー時の box 複製）**＝vtable slot 0 を copy thunk（`copy(ptr src)->ptr`＝fresh box＋`deepCopyStructValue`）にし、any→any コピー6面（let/assign/arg/return/field・`copyAnyIfPlace`＝place のみ複製）で呼ぶ＝`mut val b = a` 後の `b.bump()` が `a` に波及しない（Step C で導入した可変 existential の共有バグを修正）。tests run/any_value_semantics。**残り（任意）**＝`for val x in anyIter` 直接 desugar（現状は手動 while+match で十分動く）・concrete→any boxing の deep-copy（現状 shallow＝backend 全体の by-value 非コピーと同水準・simple/String フィールドは観測同一）・box の drop（rc/free＝backend 全体の array/String leak と同水準）。**`any P` の spec/08 callable 規則は全実装**（self-出力のみ・bound assoc/型引数入力・Self 返り再消去・inout self・Self 入力 reject・形成全束縛検査・値意味論）。
