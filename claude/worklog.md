...
🎉 **セルフホスト達成・stage0（Rust）退役済み・LLVM 単一 backend（C backend 退役・削除済み）。** 正典コンパイラ＝Plew パッケージ `compiler/src/`。構文解析は完全に共有 `@Std/Syntax` の責務（コンパイラとマクロが唯一の共有パーサを通る＝真の 1 AST）。自分自身を不動点までコンパイル。

- **モジュール DAG**：`@Std ← {Ir, Path, Ops, Diag} ← Frontend ← Backend ← entry`。`Ir.pw`＝純データ leaf（arena ノード型・`Comp`/`LlvmCtx` 等の状態型）／`Frontend.pw`＝解析核（`impl Comp`・`runFrontend()`）／`Backend.pw`＝LLVM エミッタ（`impl LlvmCtx`・`emitLlvm()`）／`Path`・`Ops`・`Diag`＝Frontend/Backend 双方が import する純 leaf（パス計算・演算子オペコード・診断/十進整形）。entry `_.pw` は `main` のみ（`./Frontend`・`./Backend` を import）。解析サブパスは `Codegen/*`（Resolve/Lower/Mono/Infer/Check/Verify/Emit…）、エミッタは `Backend/Llvm/*` の part 群。
- **バイナリ 1 本**：`compiler/plewc`（libLLVM-C をリンク）。`main`＝`runFrontend()`（`Comp` を返す）→ `emitLlvm()`。
...
- **グリーン**：`./test.sh`（run=311 panic=14 reject=293 part=11 partreject=5 skip=0・不動点・fail=0）／`./test-gen.sh`。
...
- **M3 パッケージ管理**：仕様確定＝[spec/17-packages](../spec/04-execution/17-packages.md)＋design-decisions「パッケージ」（manifest 3 項目／`/` は名前の字面・公開面はフラット（ルート `export`）／依存表記（members を `/` パスで選択・`as` は member 内のみ・`commit`）／桁数バージョン／最新互換・複数版共存・phantom 禁止／lock・ワークスペース＝複数メンバ単一 repo・1 バージョン／ソース配布／ネイティブ依存＝`[[native.<kind>]]`（ヘッダ判別・c/cpp/rust/system）・C/C++ は `sources`/`include`/`defines`/`std` 構造化（生フラグ＝ACE ゆえ不可・codegen/target は Plew 所有）・Rust は `path` で crate dir（staticlib+extern"C"・cargo 駆動）・ビルドスクリプトなし／**bin**＝公開実行ファイルを `bin` で列挙（1 lib+N bin・`/` パス・`as`・`plew run @Pkg:Name`・全 bin install 可）／**`[tools]`**＝import せず run する外部実行ファイル（プロジェクトローカル＋キャッシュ共有）／**`dev-dependencies` なし**＝tree shake で含有判定・到達駆動解決）。導入後に `@Std/Syntax` を in-tree→外部共有パッケージへ昇格。
  - **実装の現在地＝git 依存解決が end-to-end で動作（simple＋transitive）**。共有パッケージツールチェーン（すべて arena-free leaf・plewc と別ツールが共有）：
    - `Toml.pw`＝行指向 TOML サブセット parser（汎用ツリー `TomlDoc`／`Section`／`Field`／`Value`〔SVal/Arr/Inline〕・スカラは生バイト・非再帰）＝manifest/lock 両用。`Manifest.pw`＝`parseManifest`（name＋`[dependencies]` の `{ path }`/`{ git, version }`）＋`resolveDepImport`（path dep）＋`resolveGitDepImport`（git dep を lock の commit＋cache から `<cacheRoot>/src/<url>/<commit>/src/_.pw` へ）＋`loadManifestDeps`/`loadLock`/`isExternalDepImport`。`Version.pw`＝桁数モデル（`satisfies`＝書いた桁が一致・`compareVer`・`pickLatest`＝tag blob から最新互換を選択・`v`/`refs/tags/`/`^{}` 寛容）。`Lock.pw`＝`parseLock`（Toml 経由）＋`writeLock`（直書き・URL→version で決定的ソート）＋`lockCommitFor`。`Cache.pw`＝cache パス（`sanitizeUrl`/`repoDir`/`srcDir`）＋`cacheRoot`（`$PLEW_CACHE`|`$HOME/.plew/cache`・exec 経由）＋`trimEnd`。`Fetch.pw`＝git 操作（`ensureMirror`＝bare clone/fetch・`listTags`・`resolveCommit`・`materialize`＝`git archive | tar`）を `@Std/Process.execOut` で駆動。
  ...
  - **複数メジャー共存＝end-to-end で動作**：resolver は **(url, major) で dedup**（同 major は最新統合・異 major は別 lock エントリ）、loader は `lockCommitForConstraint`（`Version.satisfies` でインポータの制約一致版を選択）。**同名 export の束縛は import エッジで解決**＝loader が `import` ごとに **(importer→target) エッジ**（`Modules.importEdges`）を記録し、`findFunc` が「同名候補が複数モジュールにある時、use 側が**直接 import したモジュール**の候補を優先」（`hasImportEdge`・edge tier）。単一候補（再エクスポート/通常）は無影響＝tiebreak のみ。テスト `test-deps.sh`（leaf 1.x/2.x を 1 ビルドに同時ロード→各 mid が自版の `leafval` を束縛→1+100=101）。**型/メソッドの異版共存は spec が「API 境界で別型エラー」と制限**ゆえ free fn の共存で十分。
...
- **⚠ self-host サブセットの 2 つの書けない形**（self-host 中に Manifest.pw で踏んだ）：①**`inout` のスカラ/値 param を bare `=` 代入できない**（「cannot assign to immutable binding」・コンパイラ内に前例ゼロ＝`inout` は Comp/self/Array へ method/append で当てるのみ）→ **out-param をやめラベル付きレコードを return**（worklog「リファクタの心構え」の推奨と同じ）。②**`call(...).bytes`（String→byte view）をコール結果に直接書くと「field access only on a registered struct value」**＝backend がコール戻り値を String と判定できず特例 `isStringBytesField` を外す → **typed local に受けてから `.bytes`**（既存 loader が `val src: String = readFileBytes(...)` とするのと同じ）。
...
- **マングリングは全部アリーナ index 基準**（`pf<idx>`/`st<idx>`/`gf<k>`/`gs<k>`/`plew.g.<idx>`…）＝パッケージ名・version・モジュールパスを**一切エンコードしない**。**多版共存（同一パッケージの version 違いを 1 binary に）は今のままで OK**＝ソース配布＋whole-program 単一アリーナゆえ v1/v2 は別ソース＝別 index＝別シンボルで自然に分かれる（衝突するのは別コンパイル＆リンク方式＝Plew は採らない・前提は loader/resolver が v1≠v2 を別モジュールツリーで load すること）。**変更を迫るのは衝突でなくキャッシュ粒度**＝index は build グローバル（load 順依存）で content-stable でないため、whole-program 粒度キャッシュは可だが**パッケージ粒度のクロスプロジェクト再利用には mangle を「パッケージ commit/version＋安定シンボルキー」で名前空間化する必要**（不変 fetched 依存は lock の commit で名前空間＝content hash 計算不要・編集中の局所は content/mtime）。実装フェーズで対応＝**それまで成果物は per-project に閉じる**（[キャッシュ置き場の決定](design-decisions.md)＝fetch ソースのみグローバル一元 cache・成果物グローバル共有はこの mangle 改修後の additive）。
