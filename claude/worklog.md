...
**自動化＝`tools/methodize.py`**（検証済・バグ修正済）：all-c ファイル（全 top-level fn が `c: inout Comp` 先頭）を `python3 tools/methodize.py <file>` で一括メソッド化＋呼出点を repo 全体で `RECV.NAME(…)` に書換＋export 名を報告。**ガード**＝mixed ファイル（allc≠total）は refuse（非 c 関数の取りこぼし防止＝手当て要）。**twin（同名が複数定義＝オーバーロード）ファイルは不可**（blanket 書換が双方の呼出を巻き込む＝事前に `grep -rlE "^(export )?fn NAME\("` で確認）。バッチ手順＝twin/mixed でない all-c を複数 run→export 名を Backend の `import ./Frontend` から除去→dev-rebuild/test/reseed/不動点→commit。**メソッド化済（〜8 ファイル）**：旧Array述語・Codegen/Expr・Resolve/Operator・Mono/Index・Check/Match・Verify/Stmt・Resolve/MatchBind・Check/Conformance・Infer/Type・Mono/Collect。**残りの注意**＝`Lower/Expr`（lowerExpr/lowerExprWithCtx＝巨大 blast radius）・`Mono/Resolve`（typeRefEq/sameMangle 等 9 関数・Backend からも多用）は専用 run 推奨。`makeFieldType`(Check/Make) は twin＝手当て要。
