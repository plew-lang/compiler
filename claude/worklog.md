...
- **Bug11 ＝generic コンストラクタ戻り型推論**〔未〕：`Map.new()`（assoc fn・戻り `Map[K,V]`）や型引数なし `<Map keys=.. />` が、LHS 注釈・引数型から K,V を**推論しない**（`Map_new`/`(Map)` を未単相化 emit）。Bug10/12 と違い**命名でなく推論機能**＝別系統。回避＝明示型引数 `<Map[K,V] .../>`（動く）。状態＝未着手。
