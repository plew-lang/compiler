...
✅ **文字列→I64 パース TryFrom**（spec/12）＝`<I64.convert from="123"/>`→`Result[I64, ParseError]`〔`impl I64 as TryFrom[String]`〕。**負空間で累算**して I64.min〔絶対値が max 超〕も表現・overflow は checked `*`/`-` が panic する手前で reject・空/単独符号/非数字は ParseError。`ParseError`〔Core export・入力保持〕。新コンパイラ機能なし（キーストーンのみ）＝単一 reseed。test run tryfrom_parse〔min/max 境界・overflow・非数字・空・単独符号〕。**残（trivial additive）**＝整数の他ペア〔from I32/I16・unsigned U8/U16/U32 from U64〔truncU16/U32 intrinsic 追加＝two-phase reseed〕・cross 符号〕・他幅の文字列パース。機構は完成済で copy 増分。
...
- **一般 Chain トレイト `?.`**（現 Optional 具体・実装者は Optional のみ＝spec も Result 除外ゆえ実用価値低・**factory-as-trait-requirement キーストーン済ゆえ unblock**＝`trait Chain { type Value  fn chain()  factory fromValue/empty }` が書けるように）・**`Pow`/`**`**（float 後・spec/12＝`assoc fn pow(base: Self, exp: Exp) -> Output`・invocation 形〔bare `pow(base:,exp:)` か `Type.pow` か〕は要確認）＝各小〜中。
  ...
  - **残＝ユーザー struct を dict キーに**（`Dictionary[Pair, I64]` with `@[Hash]` derive）＝**別の大物（focused refactor）**：derive の 2-phase `plew gen` で phase 1（harness compile）が dead user main の `Dictionary[Pair]` 使用を処理し `Pair: Hash` 未生成のまま fail（chicken-and-egg）。gen mode で dead user main は emission では skip（`Backend/Llvm.pw` `c.genMode` 分岐）されるが front-end の instantiation discovery が flat-arena を GLOBAL に walk して漏らす。**完全な leak map（3 箇所・全て `Mono.pw`）**：(1) `collectGenInsts` の `c.stmts` Let 型注釈スキャン〔`val d: Dictionary[Pair,_]` の型注釈〕(2) `collectFnInsts`→`scanBlockInsts` の func body walk〔index `d[<Pair>]` が `registerMethodInst` 経由で receiver inst 登録〕(3) `collectGenInsts` の `c.exprs` Make スキャン〔dict literal `[k:v]` は ty=`Dictionary[Pair,_]` の Make に lower・`scanType(ty)`〕。(1)(2) は func/stmt 帰属で gate 可〔stmt-mark ヘルパ試作済〕だが (3) は **expr-tree marking**〔dead main の expr id を再帰マーク・`scanExprInsts` の ~20 variant walk を mirror〕が要る。**clean fix**＝gen mode で arena node を func 帰属させ dead user main を全 discovery scan から除外（or module-0 user code を gen mode で global scan から除外）。試行は revert 済。要 focused session。
