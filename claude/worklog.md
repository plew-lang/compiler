...
🎉 **セルフホスト達成・stage0（Rust）退役済み。** 正典コンパイラ＝Plew パッケージ `compiler/src/`（root `_.pw` が `part` で全パートを綴じる 1 モジュール＝`Loader`・`Ast`・`Parser/Decl`〔＝frontend driver＝ディスパッチャ＋import/export/directive＋derive 合成。宣言パーサは退役〕・`Codegen/`〔Emit/Resolve/Ops/Check/Expr/Stmt/Decl/Mono/Array/Async/Gen/Lower〕）。**レクサ・式/文パーサ・全宣言（struct/enum/fn/impl/trait/extern）パーサは `@Std/Syntax` に統合し本体は共有経路で parse→`Codegen/Lower.pw` で arena へ lower**（メタプロと同一・ドリフトする文法に二重パーサなし＝A 核心達成。`Parser/Stmt.pw`/`Parser/Expr.pw` は退役）。自分自身を不動点までコンパイル。hidden-meaning（整数幅・match 網羅・ラベル・受理の健全性・値意味論/CoW・`unique`/`deinit`/move・generics・トレイト＋Eq/Ord＋derive＋関連型・クロージャ）は概ね解消。**残る剥離・暫定は [provisional.md](provisional.md) が正典**。
...
## A（フロントエンド統合）＝核心達成・式文＋全宣言シグネチャが完全 1 本化（2026-06-06）
...
- ✅ **A-3＝impl ブロックを共有パーサ＋lowerImpl へ・旧 parseImpl/parseFuncCommon/parseWhereClause 退役**（245/245・不動点維持）。新値ツリー `MemberKind`/`MemberAst`/`ImplAst`＋共有 `parseImplDecl`/`parseMethodAfterFn`（parseImpl をメンバ単位でミラー）。`lowerImpl`/`lowerMethodMember` が全 arena 効果を順序通り再現＝メソッド/assoc-fn→Func（recv＋self mode）・witnessed sig→Conform・via→MethodAlias・`type Item`→AssocBinding・deinit→`Type_deinit`＋c.deinits・where（method-own→impl-level）→FuncBound。impl 系の名前は全て `moduleOf` 不参加（メソッドは recv＋selector、conformance は text 解決）ゆえ再インターン安全。
- ✅ **A-4＝trait + extern を共有パーサ＋lowerへ・native 型/パラメータパーサを全退役**（245/245・不動点維持）。新値ツリー `SigAst`/`TraitAst`/`ExternAst`＋`parseTraitDecl`/`parseExternDecl`。`lowerTrait`/`lowerExtern`/`lowerSigToFunc`。要求シグネチャのパース統合で **native の最後の宣言パーサ群を削除**＝`parseTrait`/`parseExtern`/`parseReqSig`/`parseParamList`/`parseTypeParams`/`parseTypeTok` を消し、**`Parser/Stmt.pw` をファイルごと退役**（唯一残った `recordArrayElem` は唯一の呼び手 `lowerType` の隣＝`Codegen/Lower.pw` へ移設）。extern fn 名は**原本 offset 維持**（cross-module intrinsic 呼び＝`moduleOf`）。

- ✅ **A5.1＝import/export/directive も共有パーサへ・1 プログラムパーサに統合**（245/245・不動点維持）。新値ツリー `TopItemAst`（Decl/Impl/Trait/Extern/Import/Export/ReExport/Directive）＋`ImportAst`/`DirectiveAst`/`DeriveReqAst`、唯一の入口 `parseProgramAst(p) -> Array[TopItemAst]`（プログラム全体）。本体 `parseProgram` は「parseProgramAst→`lowerTopItem` 各 item」だけに。`lowerImport`/`lowerDirective`/`lowerExportItem`（c.imports/c.exports/pendingDerives を原本 offset で再現）。**native の最後のパーサ＋ブリッジを退役**＝parseImport/parseExport/parseDirective/parseDirectiveArgs＋parseDeclItem/parseImplItem/parseTraitItem/parseExternItem/parseBlock/parseExprC。
- ✅ **A5.2＝パース駆動ローディング・collectParts 事前スキャンを撤廃**（245/245・不動点維持）。`parseProgramAst` が `part` も `Import`(isImport=false) ノードで返す。driver（`main`）を**再構築**＝各ファイルを c.bytes に追記→`parseAndLowerFile`（共有パーサで parse＋lower）→返った木の `Import` ノードを辿って子ファイル探索（事前 lex スキャンなし）。**退役**＝`collectParts`/`isPathTokKind`/`hasDirective`（Loader）・`parseProgram`/`expect`＋Comp カーソルメソッド 6 個（curKind/cur/peekKind/advance/skipNewlines/identIs）・`Comp.toks`/`Comp.pos` フィールド。c.bytes はファイル源とインターン名が交互（writeSpan は text 出力・moduleOf は per-file レンジゆえ不動点・可視性とも安全）。

**＝A 完了。構文解析は完全に `@Std/Syntax` の責務（文字列/トークン→木・import ノード込み）、コンパイラは木を lower するだけの意味解析。** ローダもパース駆動＝木が探索を駆動（ハンドロールのスキャナなし）。残る I/O（ファイル読込・パス解決）と arena バイト配置・module tagging はコンパイラ側＝構文解析でない部分のみ。`Parser/Decl.pw` は frontend driver は持たず（parseProgram は `main` 内 BFS に吸収）、module-tag helper（markImport/recordExport）＋derive 合成（synth*）のみ。
