...
**＝A の核心は達成（式・文・型・全宣言シグネチャ〔struct/enum/fn/impl/trait/extern〕がコンパイラとマクロで唯一の共有 @Std/Syntax パーサを通る＝ドリフトする文法に二重パーサなし）。** 共有パーサのヘッダ／`Parser/Decl.pw` のヘッダも実態へ更新済。共有パーサの旧 skip 系（`skipBlock`/`skipDefaultExpr`＝M1 の「本体スキップ」）も退役。

**意図的に本体フロントエンドに残すもの（＝import/export/directive、worklog step 5 の literal 退役は見送り）**：`parseProgram`（ディスパッチャ＝ブリッジ層）・`parseDirective`/`parseDirectiveArgs`（`@[...]`→pendingDerives）・`parseImport`/`markImport`（→c.imports）・`parseExport`/`recordExport`（→c.exports・実宣言は既に `parseDeclItem`/`parseExternItem` 経由）・derive 合成（synth*＝codegen）。**理由**：これらは式・型・宣言シグネチャを一切パースせず（directive 引数は既に共有 `parseExprC`）、**マクロ側に対応物がない＝ドリフトする二重パーサではない**。やっているのは loader と結合した module tagging（原本 offset で `moduleOf`）と derive 蓄積＝コンパイラ frontend が Comp state を直接作る仕事。@Std/Syntax（＝マクロとコンパイラが共有する「形」のパーサ）へ移すと**コンパイラ専用の値型（ImportAst/ExportAst/DirectiveAst）を共有ライブラリに足す**ことになり、共有境界の趣旨（共有するのは macro と compiler の両方が要る物のみ）に反する。ゆえに**コンパイラ frontend に置くのが正**（→ user 確認事項＝review-items に記録）。**この判断を覆すなら**：①ImportAst/ExportAst/DirectiveAst を @Std/Syntax に定義 ②parse→lower（module tagging は原本 offset 厳守）③`parseProgram` を純ディスパッチャ化、で literal「真の 1 AST」も可能（drift 価値ゼロ・@Std/Syntax 肥大の代償）。
