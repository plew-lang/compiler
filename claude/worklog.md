...
4. 🔄 **ARC ＋ `Ref`/`WeakRef`**：✅ 基本 `Ref[T]`。✅ **スコープフレーム基盤**＝`genBlock`/`for`/`match` 入口で `scopeMark`→出口で `scopeExit`（`emitScopeDrops`〔今は no-op〕＋`popLocals`）。名前解決を真の lexical scope に厳格化（回帰テスト `scope_shadow`）。✅ **ASan ハーネス**（`ASAN=1 ./test.sh`・free 導入時の UAF/double-free を機械検証・leak 検出は free 後に `detect_leaks=1`）。
   ...
   - **CoW（次の大物）**：eager copy → **共有＋retain・書込時 rc>1 で複製（uniqueness check）**。要：①束縛/格納を retain 共有に ②変異点（`arr[i]=v`・`append`・`inout`）で「rc>1 なら複製してから変異」③struct も retain/release（フィールド再帰）。`inout` の in-place は一意性ガード。検証＝fixpoint＋ASan コンパイラ。
   - CoW 後に安全化：**owned struct/enum ローカル release**・**再帰 release**（配列の struct 要素・現状 `_copy` が浅い共有なので copy も深くする必要）・**早期 exit release**（`return`〔値を temp 退避〕/`break`/`continue`・`panic` は drop 不走）・**`Ref` 箱の refcount 化**（Expr.pw の malloc）。
