...
**進行中の大物＝コンパイラ構造リファクタ**（83 フィールドの `Comp` god-object 解体＋巨大ファイル分割＋モジュール化）＝[compiler-refactor.md](compiler-refactor.md)（診断・到達点・マイルストーン M0-M4・未検証リスク）。引数名/ラベルは [spec/04 規約](../spec/01-basics/04-functions.md#引数名とラベルの指針規約) に揃える。**M0〜M3＋M5 完了**（`Comp` 11 フィールド・巨大ファイル全分割・scoped resolution）。**M4 の主目的は part 撲滅＝モジュール構造を spec/15 ベストプラクティス（既定 import・part は相互再帰核/無名 impl のみ）に沿わせる**（91 part の単一モジュール解消）。型回復統一（typed IR）は当初 B として主目標化したが、part 撲滅には不要（Backend→Frontend の型回復呼びは一方向で循環せず）と判明し**後段の任意トラックへ退避**。**M4-1/M4-2/M4-3 完了＝`Ir ← Frontend ← Backend ← entry` の一方向 DAG を達成**（commit `2ddba1b`・不動点緑 run=307）：M4-1＝IR をデータモジュール `Ir.pw` へ昇格（旧 `Ast.pw`・初の cross-module 共有可変 `Comp`・tag `refactor-m4-1`）／M4-2＝逆依存3関数（＋`stmtPrincipalOffset`）を Backend→Frontend/Ir へ pure-move し循環を断ち Backend 独立化／M4-3＝Frontend モジュール化・root `_.pw` は `main` のみ。予備作業として `findFunc` 解決を単一チョークポイント `resolvedCallee`（exprId キー永続 memo `TypeCache.callee`）へ集約済＋`TypeInfo.viewExtId` 追加（typed IR stage1）。**到達状態＝「part 最低限・spec/15 準拠」は実質達成**：残 91 part は全て Frontend/Backend 内で、依存グラフ実測により核が強連結成分（Resolve↔Check↔Mono↔Infer↔Verify↔Stmt）＝spec/15 理由②で part 正当と確認。さらなる削減は周縁モジュール抽出（小利得・誤配置ユーティリティの逆エッジ除去が前提）かメソッド化（北極星・長い裾）のみ＝詳細は compiler-refactor.md「到達状態」。
