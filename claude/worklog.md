...
- 🔲 **step 5b＝提供メソッド `.map()`/`.filter()` の sugar**（`xs.map(f:…)`）。**残ギャップ＝`Self`/関連型 `Item` を型として解決する機構**：提供メソッド `impl Iterator { fn map[B](f: fn(x: Item)->B) -> MapIter[Self, …, B] }` は instantiateProvidedMethods が retTy/params を**そのままコピー**＝`Self`/`Item` が未置換で残り、`MapIter[Self,…]` genInst が bound 検査で落ちる。要：(1) 型位置の `Self`→準拠型、`Item`→その `type Item` 束縛への解決を scan/check/codegen 横断で（メソッド emit 時に self 型 env を張る）、(2) 提供メソッド body 内の `Self`/`Item` も同 env で。現状 `Self` を**型として**解決する機構は無い（`self` 値の `isSelfRef` は別物）。
