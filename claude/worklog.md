...
- **一般 Chain トレイト `?.`**（現 Optional 具体・実装者は Optional のみ＝spec も Result 除外ゆえ実用価値低・**factory-as-trait-requirement キーストーン済ゆえ unblock**＝`trait Chain { type Value  fn chain()  factory fromValue/empty }` が書けるように）・**`Pow`/`**`**（float 後・spec/12＝`assoc fn pow(base: Self, exp: Exp) -> Output`・invocation 形〔bare `pow(base:,exp:)` か `Type.pow` か〕は要確認）＝各小〜中。
  ...
  - **残＝ユーザー struct を dict キーに**（`Dictionary[Pair, I64]` with `@[Hash]` derive）＝**2 つの sub-blocker**。(a) **gen-discovery〔解法判明・実証済〕**＝gen の phase 1 で `Dictionary[Pair]` が `Pair: Hash` 未生成のまま instantiate されて fail（chicken-and-egg）。**`scanType` の genInst 登録点（`Mono.pw:900`）に単一 gate**＝gen mode で「type arg が derive 対象（`c.derives` に typeStart 一致）を名指す generic inst」は登録しない〔`refArgNamesADeriveTarget`〕。これで phase 1 が `App.gen.pw`〔`impl Pair as Hash`〕を生成・phase 2 も compile OK。〔3 leak 点を flat-arena で個別 gate する node-marking 案は不要＝scanType の単一点で足りる。試作 revert 済だが解法は確定〕。(b) **runtime struct-key segfault〔別の深い backend bug・残〕**＝phase 2 は compile するが `d[k]`（lookup）で SIGSEGV。crux＝monomorphized `Dictionary[Pair].find` 内の `self.keys[idx] == key`〔`std/Prelude.pw:309`〕＝**generic method context で struct 型 K の array 要素 `self.keys[idx]` を K→Pair に ground し損ねる**（要素 read のサイズ/型 or `==` witness 経由が template K のまま）。**標準 `Array[Pair]` の要素 `==` は main で動く**ので generic-method 単相化文脈固有〔`arrFieldElemStructIdx` は struct idx を返すはずだが要素 read の LLVM 型 or witness 引数が誤る〕。両方解いて初めて feature 完成＝(a)+(b) で focused session。
