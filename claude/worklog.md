...
- **グリーン**：`./test.sh`（run259/panic12/reject267/part5/fail=0＋不動点）／`./test-gen.sh`（gen10/reject1）。
...
- **literal match パターン**（`match x { 0 => …  _ => … }`／`"yes" => …`・spec/11「bare リテラルはマッチ」）＝中。現状 enum/struct/wildcard のみ。**設計案＝lowering 脱糖**（literal-pattern match を temp-let＋if-else 連鎖へ・既存 `==`/if codegen を再利用＝backend match emitter を触らず安全。scrut 単一評価のため temp 束縛が要る）。parser〔Int/Str/Bool token をパターンに〕＋PatternAst リテラルフィールド＋網羅性〔wildcard 必須〕。
...
- **名前空間 import `Io.print`**＝中（解決系に侵襲的）。`import @Std/Io`〔no `with`〕で名前空間 `Io` 束縛・`Io.print` を qualified call に解決・`import … as F` リネーム束縛・`_.pw` ディレクトリ解決・パス正当性厳密検査。
...
- **一般 Chain トレイト `?.`**（現 Optional 具体）・**`Pow`/`**`**（float 後）・**`Output != Self`**・**式位置 `panic`**（文位置は✅）・**`guard` 文**（条件チェーン束縛・parser 未）・**match ガード/ネストパターン**・**global 前方参照**・Bool dict キー（低価値）＝各小〜中。
