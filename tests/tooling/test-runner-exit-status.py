#!/usr/bin/env python3
"""Exercise the actual tests/harness/test.sh workers with controlled compiler/linker doubles."""
import os
...
def install_supervisors(root):
    ...
    for name in ('scripts/support/watch-command.py', 'scripts/support/trace-command.py', 'scripts/support/process_watch.py'):
...
with tempfile.TemporaryDirectory(prefix='plew-gen-status-') as directory:
    ...
    source = (Path(__file__).resolve().parents[2] / 'tests/harness/test-gen.sh').read_text()
    blocks = {
...
with tempfile.TemporaryDirectory(prefix='plew-asan-status-') as directory:
    ...
    for name, body, expected_pass in cases:
        for stdin in (False, True):
            ...
            # This gate checks memory/exit status; golden comparison is tests/harness/test.sh's job.
...
deps = (Path(__file__).resolve().parents[2] / 'tests/harness/test-deps.sh').read_text()
