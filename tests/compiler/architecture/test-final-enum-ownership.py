...
source = (Path(__file__).resolve().parents[3] / 'src/Backend/Llvm/Enums.pw').read_text()
...
arrays = (Path(__file__).resolve().parents[3] / 'src/Backend/Llvm/Arrays.pw').read_text()
...
ownership = (Path(__file__).resolve().parents[3] / 'src/Codegen/Mono/Ownership.pw').read_text()
...
for name in ('Mid', 'Arrays', 'Any', 'ArrayOps', 'CallGeneric'):
    body = (Path(__file__).resolve().parents[3] / f'src/Backend/Llvm/{name}.pw').read_text()
