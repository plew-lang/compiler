...
2. [型システム](spec/02-types.md) — プリミティブ型、文字列（不変・UTF-8・変数展開）、複合型（配列／辞書／タプル／レンジ）、構造体／列挙型／トレイト、フィールド統一原則、Optional／Result、インスタンス生成（JSX／factory）、newtype（名目型）、ジェネリクス、where 句
